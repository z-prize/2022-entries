                             ...
                             size_t ffi_affine_sz)
{
	...
	for (; work < (int)batches; work++) {
	  ...
	  batch_pool.spawn([&]() {
	      ...
	      cudaEvent_t start, kernel_stop, transfer_stop;
	      ...
	      float kernel_milliseconds, transfer_milliseconds = 0;
	      cudaEventElapsedTime(&kernel_milliseconds, start, kernel_stop);
	      // printf("add mix time = %fms\n", kernel_milliseconds);
	      cudaEventElapsedTime(&transfer_milliseconds, kernel_stop, transfer_stop);
	      // printf("transfer time = %fms\n", transfer_milliseconds);
	  ...
	  batch_pool.spawn([&]() {
		...
		// printf("accumulate time time = %fms\n", milliseconds);
...
// Allocate device storage, transfer bases
extern "C"
RustError mult_pippenger_init_edwards(RustContext<bucket_t, affine_t, scalar_t> *context,
                              const affine_t points[], size_t npoints,
                              size_t ffi_affine_sz)
{
    context->context = new Context<bucket_t, affine_t, scalar_t>();
...
// Peform MSM on a batch of scalars over fixed bases
extern "C"
RustError mult_pippenger_inf_edwards(RustContext<bucket_t, affine_t, scalar_t> *context,
                             point_t* out, const affine_t points[],
                             size_t npoints, size_t batches,
                             const scalar_t scalars[],
                             size_t ffi_affine_sz)
