...
int32_t MSMContext::msmRun(uint64_t* projectiveResultsPtr, void* scalarsPtr,  uint32_t scalars) {
  ...
  void*     partition1024Args[5]={&ml.pages, &ml.sizes, &ml.counters, &ml.processedScalars, &currentPoints};
  ...
  for(uint32_t batch=0;batch<=batches;batch++) {    
    ...
    processSignedDigitsKernel<<<currentPoints/256, 256, 8928, runStream>>>(ml.processedScalars, ml.scalars, currentPoints);
    ...
    if(batch==0) {
      ...
      computeBucketSums<true><<<smCount, 384, 96*1024, runStream>>>(ml.buckets, gpuPointsMemory, ml.sortedTriple, ml.points, ml.atomics);
    }
    else if(batch==1) {
      // special case -- 3 quarters of first scalars -- accumulate to existing buckets
      computeBucketSums<false><<<smCount, 384, 96*1024, runStream>>>(ml.buckets, gpuPointsMemory, ml.sortedTriple, ml.points, ml.atomics);
