...
template cudaError_t allocate<ff_storage<12u>>(allocation<ff_storage<12u>> &allocation, const size_t size, cudaMemPool_t pool, cudaStream_t stream);
template cudaError_t free<ff_storage<12u>>(allocation<ff_storage<12u>> &allocation, cudaStream_t stream);
