...
private:
    ...
    static inline void mul_n(uint32_t* acc, const uint32_t* a, uint32_t bi,
                             size_t n=n)
    {
		//const uint64_t* a1 = (const uint64_t*)acc;
...
/*			mont_t r;
		...
		#pragma unroll
			...
			uint32_t c = 0, s = 0, q = 0, t = 0;
			for (size_t i = 0; i < n; i++) {
				__mad_lo_cc(s, a[0], b[i], res[0]);
				__madc_hi(t, a[0], b[i], 0);

				q = M0 * s;
				__mad_lo_cc(s, MOD[0], q, s);
				__madc_hi_cc(c, MOD[0], q, t);
				__addc_cc(t, 0, 0);
				...
				for (size_t j = 1; j < n; j++) {
					...
					__mad_lo_cc(s, MOD[j], q, s);
					__madc_hi_cc(c, MOD[j], q, c);
					__addc(t, 0, 0);

					__add_cc(res[j-1], s, res[j]);

				}
				__addc_cc(res[n-1], res[n], c);
				__addc(res[n], res[n+1], t);

				res[n+1] = 0;
			}
			
			bool cond = (res[n+1] != 0) || (res[n] != 0);
			if (!cond)
			{
				uint32_t tmp;
				__sub_cc(tmp, MOD[0], res[0]);
#pragma unroll
				for (size_t i = 1; i < n; i++)
					__subc_cc(tmp, MOD[i], res[i]);
				
				  __subc(tmp, (uint32_t)0, (uint32_t)0);
				cond = (tmp > 0) ? 1 : 0;
			}
			...
			if(cond)
			{
				__sub_cc(r[0], res[0], MOD[0]);
#pragma unroll
				for (size_t i = 1; i < n; i++) 
					__subc_cc(r[i], res[i], MOD[i]);
			} 
			else 
			{
#pragma unroll
...
#pragma unroll			
			for (size_t i = 0; i < n; i++) 
			{
				for (size_t j = 0; j < i; j++) 
				{
					...
					__addc(t, t, (uint32_t)0);

					__mad_lo_cc(v, res[j], MOD[i-j], v);
					__madc_hi_cc(u, res[j], MOD[i-j], u);
					__addc(t, t, (uint32_t)0);
				}
				...
				__mad_lo_cc(v, res[i], MOD[0], v);
				__madc_hi_cc(u, res[i], MOD[0], u);
				__addc(t, t, (uint32_t)0);
				v = u;
				u = t;
				t = 0;
...
#pragma unroll
			for (size_t i = 0; i < n; i++) {
				...
				__addc_cc(r1, r1, (uint32_t)0);
				__addc(r2, r2, (uint32_t)0);
				__mad_hi_cc(r[0+1], a[0], b[i], r[0+1]);

				for (size_t j = 1; j < n-1; j++) 
					__madc_hi_cc(r[j+1], a[j], b[i], r[j+1]);
				
				__madc_hi_cc(r1, a[n-1], b[i], r1);
				__addc(r2, r2, (uint32_t)0);

				// Reduction 
				uint32_t q = M0 * r[0];

				__mad_lo_cc(r[0], MOD[0], q, r[0]);
				for (size_t j = 1; j < n; j++) 
					__madc_lo_cc(r[j], MOD[j], q, r[j]);
				
				__addc_cc(r1, r1, (uint32_t)0);
				__addc(r2, r2, (uint32_t)0);

				__mad_hi_cc(r[0], MOD[0], q, r[0+1]);
				for (size_t j = 1; j < n-1; j++) 
					__madc_hi_cc(r[j], MOD[j], q, r[j+1]);
				
				__madc_hi_cc(r[n-1], MOD[n-1], q, r1);
				__addc(r1, r2, (uint32_t)0);
				r2 = 0;
			}
			...
			{
				uint32_t tmp;
				__sub_cc(tmp, MOD[0], r[0]);
#pragma unroll
				for (size_t i = 1; i < n; i++)
					__subc_cc(tmp, MOD[i], r[i]);
			...
			if(cond)
			{
				__sub_cc(r[0], r[0], MOD[0]);
#pragma unroll
				for (size_t i = 1; i < n; i++) 
					__subc_cc(r[i], r[i], MOD[i]);				
			}	
			return r;*/
