...
__global__ void pippenger_group_NAF(const affine_t* points, bucket_t *buckets, const uint32_t* idx, const uint32_t* tableS, const uint32_t* tableE, const uint32_t* numbers, int wnum, int bucketSize);
...
__global__ void initGroup_NAF(const scalar_t* scalars, unsigned int* idx, part_t* keys, int npoints);
...
__device__ void madd_base(const fp_t& X2, const fp_t& Y2, bucket_t& p31, int& resInf, bool subtract)
{
	if (X2.is_zero())
		if (Y2.is_zero())
			return;
...
void pippenger_group_NAF(const affine_t* points, bucket_t *buckets, 
						 ...
						 const uint32_t* numbers, int wnum, int bucketSize)
{	
	...
	if (idxBucket > 0)
	{
		...
		{		
			...
			auto X = points[idx].X;
			auto Y = points[idx].Y;
			madd_base(X, Y, result, resInf, isNeg);
		}
...
#pragma unroll	
	for (int group = 0; group < NWINS; ++group)
	{
		...
		idx[id + npoints * group] = diff >= 0 ? id : -id;
	}
}
...
public:
	...
	static void madd_base(const fp_t& X2, const fp_t& Y2, bucket_t& p31, int& resInf)
	{
		if (X2.is_zero())
			if (Y2.is_zero())
				return;
    ...
    MSMConfig init_msm(size_t npoints)
	{
		...
		config.oneWaveBuckets = totalThreads / TILING;
...
//#pragma omp parallel for reduction(+: bitCount) reduction(max: max) reduction(min: min)		
		for (int z = 0; z < config.npoints; ++z)
		{
			...
			vector<int> tmp(254), tmp1(254);
			for (int z = 0; z < 254; ++z)
				tmp1[z] = tmp[z] = 0;
			
			for (int z = 0; z < 254; ++z)
			{
				int valP = get_wval(NP, z, 1);
				int valN = get_wval(NM, z, 1);
...
#endif
		if (bucketsDone == 0 && numIter == 0) // first init
			CUDA_OK(cudaMemset(d_buckets, 0, sizeof(bucket_t) * bucket_size * config.oneWaveBuckets));
		
#if NAF		
		initGroup_NAF<<<xBlock, 256, 0, streams[numStr]>>>(d_scalars, idxs, part, npoints);
#else
		initGroup_full2<<<xBlock, 256, 0, streams[numStr]>>>(d_scalars, idxs, part, npoints, NULL);
#endif
		...
		for (int group = NWINS - 1, k = 0; group >= 0; --group, ++k)
		{
			...
			sort(idxs + npoints * group, part + npoints * group, npoints, streams[numStr]);
			...
			if (group == 2)
			{
				...
				CUDA_OK(cudaEventElapsedTime (&sortTime, events[2], events[3]));
				CUDA_OK(cudaEventRecord (events[4], streams[numStr]));				
#endif		
			}*/
#if NAF			
			pippenger_group_NAF<<<xBlockP, thP, 0, streams[numStr]>>>(d_points, d_buckets + bucket_size * bucketsDone, rowsA[numStr], tableS_A[numStr], tableE_A[numStr], idxs + npoints * group, group, realTab);
#else
			//pippenger_group<<<xBlockP, thP, 0, streams[numStr]>>>(d_points, d_buckets + bucket_size * bucketsDone, rowsA[numStr], tableS_A[numStr], tableE_A[numStr], idxs + npoints * group, group);
			if (group != NWINS - 1 || (NBITS % WBITS) == 0)
				pippenger_group<<<xBlockP, thP, 0, streams[numStr]>>>(d_points, d_buckets + bucket_size * bucketsDone, rowsA[numStr], tableS_A[numStr], tableE_A[numStr], idxs + npoints * group, group);
			else
				pippenger_group_last<<<xBlockP, thP, 0, streams[numStr]>>>(d_points, d_buckets + bucket_size * bucketsDone, rowsA[numStr], tableS_A[numStr], tableE_A[numStr], idxs + npoints * group);
#endif
			...
			/*if (group == 2)
			{
				...
				CUDA_OK(cudaEventElapsedTime (&mainTime, events[4], events[5]));
			...
			if (bucketsDone == config.oneWaveBuckets)
			{
...
#endif
				pippenger_final<<<(TILING * bucketsDone) / thF, thF, 0, streams[0]>>>(d_buckets, d_none, 0);
				...
				CUDA_OK(cudaMemsetAsync(d_buckets, 0, sizeof(bucket_t) * bucket_size * config.oneWaveBuckets, streams[0]));
...
#endif	
				thread* accAsyncJob = new thread(&pippenger_t::intergateAsync, this, allDone, allDone + bucketsDone);
		...
		if (bucketsDone && scalars == NULL) // last
		{
			for (int z = 0; z < numStrs; ++z)
				CUDA_OK(cudaStreamSynchronize(streams[z]));				
#if PROF
		...
		CUDA_OK(cudaEventRecord (events[0]));
#endif
		pippenger_final<<<(TILING * NWINS) / thF, thF, 0, streams[0]>>>(d_buckets, d_none, 0);
		CUDA_OK(cudaMemcpy(c_none_ptrs[0] + TILING * 2 * NWINS * numIter, d_none, NWINS * sizeof(d_none[0]), cudaMemcpyDeviceToHost));
...
#pragma omp parallel for 	
	...
	void intergateAsync(int start, int end) 
	{
		bucket_t (*c_none)[TILING][2] = reinterpret_cast<decltype(c_none)>(c_none_ptrs[0]);
#pragma omp parallel for
