...
#pragma omp parallel for reduction(+: bitCount) reduction(max: max) reduction(min: min)	reduction(max: gapMax)
		...
		for (int group = NWINS - 1, k = 0; group >= 0; --group, ++k)
		{
			...
			{
				if (isFull || (!isFull && isLastIfNotFull))
			...
			else
			{
				...
				pippenger_group_last_NAF<<<xBlockP, thP, 0, currSt>>>(d_points, d_bucket_tmp, rowsA[numStr], tableS_A[numStr], tableE_A[numStr], idxs + npoints * group, continueAdd, elemsInBucket, threadCount);
				bucketsDone--;
