...
__global__ void pippenger_group_NAF_affine(const affine_t* points, fp_t *buckets, const uint32_t* idx, const uint32_t* tableS, const uint32_t* tableE, const uint32_t* numbers, int bucketSize, bool continueAdd);
...
__global__ void test_kernel(affine_t* points, bucket_t *buckets, const uint32_t* idx, const uint32_t* tableS, const uint32_t* tableE, const uint32_t* numbers, int bucketSize, bool continueAdd);
...
#ifdef __CUDA_ARCH__ && (KERNELS_OFF == 0)
...
void test_kernel(affine_t* points, bucket_t *buckets, 
...
__global__
//__launch_bounds__(64, 12)
void pippenger_group_NAF_affine(const affine_t* points, fp_t *buckets, 
							    const uint32_t* idxs, const uint32_t* tableS, const uint32_t* tableE,
								const uint32_t* numbers, int bucketSize, bool continueAdd)
{	
	const int globalId = threadIdx.x + blockIdx.x * blockDim.x;
	const int id = SIZE * globalId;	
	...
	while (gotCount)
	{
		...
		for (int t = 0; t < SS; ++t)
		{
			...
			while (!got)
			{
				...
				int idx = numbers[tableS[idxs[id + t]] + currS[t]];// table[k][p + t][currS[t]];
				bool isNeg = idx < 0;
				if (isNeg) 
					idx = -idx;
				idx = idx - 1;
				...
				if (X[t].is_zero() && points[idx].Y.is_zero())			
					continue;
		...
		if (gotCount == 0)
		{
			for (int t = 0; t < SS; ++t)
			{
				...
				if (idxBucket > 0)
				{
					buckets[2 * (idxBucket - 1)] = tmpS[t].X;
					buckets[2 * (idxBucket - 1) + 1] = tmpS[t].Y;
				}
			}
			continue;
		}
		...
		fp_t inv = partlyProd; //1 / partlyProd;
...
#pragma omp parallel for reduction(+: err)			
...
#if 0		
		{
			...
			
			{
				...
				double t = omp_get_wtime();
				int LL = 1000000;
//#pragma omp parallel for firstprivate(res, resInf, XX, XY)
				for (int k = 0; k < LL; ++k)
				{
					fp_t INV = XY[1] - XY[3];
					INV *= XY[1];
...
//#pragma omp parallel for reduction(+: bitCount) reduction(max: max) reduction(min: min)	
		for (int z = 0; z < config.npoints; ++z)
		{
			...
			for (int z = 0; z < nWin; ++z)
			{
				int val = get_wval(s, z * wBit, (z == nWin - 1) ? (253 % wBit) : wBit);
				distr[z][val]++;
			}
		}
...
//#pragma omp parallel for reduction(+: bitCount) reduction(max: max) reduction(min: min)		
		for (int z = 0; z < config.npoints; ++z)
		{
			...
			for (int z = 0; z < nWin; ++z)
			{
				int valP = get_wval(NP, z * wBit, (z == nWin - 1) ? (253 % wBit) : wBit);
				int valN = get_wval(NM, z * wBit, (z == nWin - 1) ? (253 % wBit) : wBit);
		...
		for (int group = NWINS - 1, k = 0; group >= 0; --group, ++k)
		{
			...
			sort(idxs + npoints * group, part + npoints * group, npoints, currSt, (group == NWINS - 1) ? &elemsInBucket : NULL);
			...
			{
				//pippenger_group_NAF_affine<<<xBlockP / SIZE, thP, 0, streams[numStr]>>>(d_points, (fp_t*)(d_buckets + realTabAligned * bucketsDone), rowsA[numStr], tableS_A[numStr], tableE_A[numStr], idxs + npoints * group, realTab, continueAdd);
