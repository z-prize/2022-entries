...
public:
    ...
    inline mont_t& operator-=(const mont_t& b)
    {
        ...
        asm("{ .reg.pred %top; .reg.u32 %b;");
        asm("subc.u32 %b, 0, 0; setp.ne.u32 %top, %b, 0;");
        asm("@%top add.cc.u32 %0, %0, %1;" : "+r"(even[0]) : "r"(MOD[0]));
        for (i = 1; i < n-1; i++)
            asm("@%top addc.cc.u32 %0, %0, %1;" : "+r"(even[i]) : "r"(MOD[i]));
        asm("@%top addc.u32 %0, %0, %1;" : "+r"(even[i]) : "r"(MOD[i]));
        asm("}");
