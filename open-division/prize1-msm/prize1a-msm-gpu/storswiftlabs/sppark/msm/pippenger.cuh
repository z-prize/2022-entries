...
void pippenger(const affine_t* points, size_t npoints,
               ...
               bucket_t (*ret)[NWINS][NTHREADS>>ACC_LEVEL][2] = nullptr);
__global__ void pre_scalars(scalar_t* scalars, const size_t npoints);
#ifdef __CUDA_ARCH__
...
__global__
void pre_scalars(scalar_t* scalars_, const size_t npoints)
{
    size_t gid = blockIdx.x *256 + threadIdx.x;
    #pragma unroll 1
    for (; gid<npoints; gid+=blockDim.x*256) {
        scalar_t s = scalars_[gid];
        s.from();
        scalars_[gid] = s;
    }   
}
...
static point_t integrate_row(const bucket_t row[NTHREADS>>ACC_LEVEL][2], int wbits = WBITS)
{
...
template<class bucket_t> class result_t {
    bucket_t ret[NWINS][NTHREADS>>ACC_LEVEL][2];
public:
    result_t() {}
...
public:
    ...
    size_t get_size_dones(MSMConfig& config) {
        return config.N * sizeof(bucket_t) * NWINS * (NTHREADS>>ACC_LEVEL) * 2;
    }
    ...
    void transfer_bases_to_device(MSMConfig& config, size_t d_bases_idx, const affine_t points[],
        ...
        if (ffi_affine_sz != d_affine_sz)
            CUDA_OK(cudaMemcpy2DAsync(d_points, d_affine_sz,
                                      ...
                                      min(d_affine_sz,ffi_affine_sz), config.npoints,
    ...
    void launch_kernel(MSMConfig& config,
                       ...
                       size_t d_dones_idx, bool mont = true, cudaStream_t s = nullptr)
    {
        ...
        bucket_t (*d_none)[NWINS][NTHREADS>>ACC_LEVEL][2] = 
            reinterpret_cast<decltype(d_none)>(d_bucket_ptrs[d_dones_idx]);
        CUDA_OK(cudaSetDevice(device));

        if (mont) {
            launch_coop(pre_scalars, sm_count, 256, stream, d_scalars, config.npoints);
        }
