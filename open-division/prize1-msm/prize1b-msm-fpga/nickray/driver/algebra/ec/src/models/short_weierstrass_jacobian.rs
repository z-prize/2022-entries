...
use crate::{
    // models::SWModelParameters as Parameters, msm::{Msm, VariableBaseMSM}, AffineCurve, ProjectiveCurve,
    models::SWModelParameters as Parameters, AffineCurve, ProjectiveCurve,
    DoubleInPlace,
};
...
impl<P: Parameters> core::iter::Sum<Self> for GroupAffine<P> {
    fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
        iter.fold(GroupProjective::<P>::zero(), |sum, x| sum + &x)
            .into()
    }
}
...
impl<'a, P: Parameters> AddAssign<&'a GroupAffine<P>> for GroupProjective<P> {
    fn add_assign(&mut self, other: &'a GroupAffine<P>) {
        if other.is_zero() {
            return;
        }
