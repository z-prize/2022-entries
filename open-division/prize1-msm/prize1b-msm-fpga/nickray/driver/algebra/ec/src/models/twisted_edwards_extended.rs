...
impl<P: Parameters> DoubleInPlace for GroupProjective<P> {
    fn double_in_place(&mut self) -> &mut Self {
...
impl<P: Parameters> core::ops::ShlAssign<usize> for GroupProjective<P> {
    fn shl_assign(&mut self, c: usize) {
        (0..c).for_each(|_| { self.double_in_place(); })
    }
}

impl<P: Parameters> core::ops::Shl<usize> for GroupProjective<P> {
    type Output = Self;
    fn shl(mut self, c: usize) -> Self {
        self <<= c;
        self
    }
}
...
impl<'a, P: Parameters> AddAssign<&'a GroupAffine<P>> for GroupProjective<P> {
    fn add_assign(&mut self, other: &GroupAffine<P>) {
        // See "Twisted Edwards Curves Revisited"
        // Huseyin Hisil, Kenneth Koon-Ho Wong, Gary Carter, and Ed Dawson
        // 3.1 Unified Addition in E^e
...
impl<P: Parameters> Add<&GroupAffine<P>> for GroupProjective<P> {
    type Output = Self;
    fn add(mut self, other: &GroupAffine<P>) -> Self {
        self += other;
        self
    }
}

impl<'a, P: Parameters> SubAssign<&'a GroupAffine<P>> for GroupProjective<P> {
    fn sub_assign(&mut self, other: &'a GroupAffine<P>) {
...
// impl<P: Parameters> Msm<GroupAffine<P>> for GroupProjective<P> {
//
//     type Scalar = <Self as ProjectiveCurve>::ScalarField;
//
// }
//
// impl<P: Parameters> VariableBaseMSM<GroupAffine<P>> for GroupProjective<P> {}


/// Affine coordinates for a point on a twisted Edwards curve, over the
/// base field `P::BaseField`, with t = xy and kt precomputed.
#[derive(Derivative)]
#[derivative(
    Copy(bound = "P: Parameters"),
    Clone(bound = "P: Parameters"),
...
#[must_use]
pub struct PreGroupAffine<P: Parameters> {
    /// X coordinate of the point represented as a field element
    pub x: P::BaseField,
    /// Y coordinate of the point represented as a field element
    pub y: P::BaseField,
...
impl<P: Parameters> PreGroupAffine<P> {
    pub const fn junk(j: P::BaseField) -> Self {
        Self { x: j, y: j, kt: j }
    }
}

impl<P: Parameters> From<GroupAffine<P>> for PreGroupAffine<P> {
    fn from(affine: GroupAffine<P>) -> PreGroupAffine<P> {
...
impl<P: Parameters> From<PreGroupAffine<P>> for GroupAffine<P> {
    fn from(pre: PreGroupAffine<P>) -> GroupAffine<P> {
...
impl<P: Parameters> PreGroupAffine<P> {
    pub fn new(x: P::BaseField, y: P::BaseField) -> Self {
        Self { x, y, kt: (P::COEFF_D+P::COEFF_D)*x*y  }
    }
}

// impl<P: Parameters> crate::msm::Msm<PreGroupAffine<P>> for GroupProjective<P> {
//
//     type Scalar = <Self as ProjectiveCurve>::ScalarField;
//
// }
//
// impl<P: Parameters> crate::msm::VariableBaseMSM<PreGroupAffine<P>> for GroupProjective<P> {}

impl<'a, P: Parameters> AddAssign<&'a PreGroupAffine<P>> for GroupProjective<P> {
    fn add_assign(&mut self, other: &PreGroupAffine<P>) {
        if self.is_zero() { // not checking this results is 7M vs 1M
            self.x = other.x;
            self.y = other.y;
...
impl<'a, P: Parameters> SubAssign<&'a PreGroupAffine<P>> for GroupProjective<P> {
    fn sub_assign(&mut self, other: &'a PreGroupAffine<P>) {
        self.add_assign(&(-*other))
    }
}

impl<P: Parameters> Neg for PreGroupAffine<P> {
    type Output = Self;

    fn neg(self) -> Self {
        Self { x: -self.x, y: self.y, kt: -self.kt }
    }
}

/// The same group as GroupProjective, but with alternative addition formulae: the "4-processor".
#[derive(Derivative)]
#[derivative(
    Copy(bound = "P: Parameters"),
    Clone(bound = "P: Parameters"),
...
#[must_use]
pub struct ParGroupProjective<P: Parameters> {
    pub x: P::BaseField,
    pub y: P::BaseField,
    pub t: P::BaseField,
    pub z: P::BaseField,
...
impl<P: Parameters> ParGroupProjective<P> {
    pub fn new(x: P::BaseField, y: P::BaseField, t: P::BaseField, z: P::BaseField) -> Self {
        Self { x, y, t, z }
    }
}
impl<P: Parameters> Zeroize for ParGroupProjective<P> {
    // The phantom data does not contain element-specific data
    // and thus does not need to be zeroized.
    fn zeroize(&mut self) {
        self.x.zeroize();
...
impl<P: Parameters> Zero for ParGroupProjective<P> {
    fn zero() -> Self {
        Self::new(
            P::BaseField::zero(),
            P::BaseField::one(),
...
impl<P: Parameters> From<GroupProjective<P>> for ParGroupProjective<P> {
    fn from(point: GroupProjective<P>) -> ParGroupProjective<P> {
        ParGroupProjective {
            x: point.x,
            y: point.y,
            t: point.t,
...
impl<P: Parameters> From<ParGroupProjective<P>> for GroupProjective<P> {
    fn from(point: ParGroupProjective<P>) -> GroupProjective<P> {
        GroupProjective {
            x: point.x,
            y: point.y,
            t: point.t,
...
impl<P: Parameters> PartialEq for ParGroupProjective<P> {
    fn eq(&self, other: &Self) -> bool {
        if self.is_zero() {
            return other.is_zero();
        }
...
impl<P: Parameters> Distribution<ParGroupProjective<P>> for Standard {
    #[inline]
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> ParGroupProjective<P> {
        let point: GroupProjective<P> = self.sample(rng);
        point.into()
    }
}

ark_ff::impl_additive_ops_from_ref!(ParGroupProjective, Parameters);

impl<'a, P: Parameters> Add<&'a Self> for ParGroupProjective<P> {
    type Output = Self;
    fn add(mut self, other: &'a Self) -> Self {
        self += other;
        self
...
impl<'a, P: Parameters> AddAssign<&'a Self> for ParGroupProjective<P> {
    #[allow(non_snake_case)]
    fn add_assign(&mut self, other: &'a Self) {
        // See "Twisted Edwards Curves Revisited" (https://eprint.iacr.org/2008/522.pdf)
        // by Huseyin Hisil, Kenneth Koon-Ho Wong, Gary Carter, and Ed Dawson
        // 4.2 $-processor twisted Edwards (a=-1) unified addition in E^e

        // step 1
        let mut R1 = self.y - self.x;
        let mut R2 = other.y - other.x;
        let mut R3 = self.y + self.x;
        let mut R4 = other.y + other.x;

        // step 2: 1M (if parallel)
        let R5 = R1 * R2;
        let R6 = R3 * R4;
        let mut R7 = self.t * other.t;
        let mut R8 = self.z * other.z;
...
impl<'a, P: Parameters> SubAssign<&'a Self> for ParGroupProjective<P> {
    fn sub_assign(&mut self, other: &'a Self) {
        *self += &(-(*other));
    }
}

impl<P: Parameters> Neg for ParGroupProjective<P> {
    type Output = Self;
    fn neg(mut self) -> Self {
        self.x = -self.x;
        self.t = -self.t;
