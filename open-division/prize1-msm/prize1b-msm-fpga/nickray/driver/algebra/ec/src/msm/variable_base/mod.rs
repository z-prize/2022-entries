use core::ops::{Neg, Shl, ShlAssign};
...
// Would need adapting to new MSM trait
//
pub mod stream_pippenger;
pub use stream_pippenger::*;

/// Requirements to implement MSM
pub trait Msm<Input: Copy+Neg>:
    Eq
    + Sized
    + Sync
    + Zero
    ...
    // the "mixed" add/sub
    + for<'a> AddAssign<&'a Input>
    + for<'a> SubAssign<&'a Input>
    // doubling
...
/// Default implementation of variable base MSM
pub trait VariableBaseMSM<Input: Copy+Neg>: Msm<Input>
{
    ...
    ///
    /// </section>
    fn msm(bases: &[Input], scalars: &[Self::Scalar]) -> Self {
    ...
    fn msm_checked(bases: &[Input], scalars: &[Self::Scalar]) -> Result<Self, usize> {
