...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    /// Add by ars
    /// Merge combined_comms and adjusted_witnes msm bases, scalars
    /// Merge witnes msm bases, scalars
    pub fn ars_batch_check<'a>(
        vk: &VerifierKey<E>,
        commitments: Vec<(String, Vec<E::G1Affine>, Vec<E::Fr>, Option<usize>)>,
        query_set: &QuerySet<E::Fr>,
        values: &Evaluations<E::Fr>,
        proof: &BatchProof<E>,
        fs_rng: &mut S,
        ...
        where
            Commitment<E>: 'a,
    {
        let commitments: BTreeMap<_, _> = commitments.into_iter().map(|c| (c.0.clone(), c)).collect();
        let mut query_to_labels_map = BTreeMap::new();
        ...
        let mut fs_rng_list: Vec<Vec<<E as PairingEngine>::Fr>> = vec![];
        let mut query_to_labels_list = Vec::new();
        let mut randomizer_list = Vec::new();
        let mut cnt = 0;
        let acc_time = start_timer!(|| format!("#### randomizer_list"));

        // Use fs_rng finds the randomizer
        for v in query_to_labels_map.into_iter() {
            let mut fs_rng_vec: Vec<<E as PairingEngine>::Fr> = vec![];
            for _ in v.1.1.iter() {
                fs_rng_vec.push(fs_rng.squeeze_short_nonnative_field_element().unwrap());
            }
            fs_rng_list.push(fs_rng_vec);
            query_to_labels_list.push(v);

            if cnt == 0 {
                randomizer_list.push(E::Fr::one());
            } else {
                randomizer_list.push(fs_rng.squeeze_short_nonnative_field_element().unwrap());
            }
            cnt += 1;
        }
        end_timer!(acc_time);

        let mut combined_witnes_base = vec![];
        let mut combined_witnes_scalar = vec![];

        let mut msm_map: BTreeMap<
            Option<usize>,
            (Vec<E::G1Affine>, Vec<<<E as PairingEngine>::Fr as PrimeField>::BigInteger>),
        > = BTreeMap::new();

        let acc_time = start_timer!(|| "##### Accumulating elements prepare");
        for ((((_query_name, (query, labels)), p), randomizer), fs_rng_vec) in
        query_to_labels_list.into_iter().zip_eq(&proof.0).zip_eq(randomizer_list).zip_eq(fs_rng_list)
        {
            ...
            for label in labels.into_iter() {
                let commitment =
                    commitments.get(label).ok_or(PCError::MissingPolynomial { label: label.to_string() }).unwrap();
                let v_i = values
                    .get(&(label.clone(), *query))
                    .ok_or(PCError::MissingEvaluation { label: label.to_string() })
                    .unwrap();
            ...
            for (((_label, bases, scalars, degree_bound), value), curr_challenge) in
            comms_to_combine.into_iter().zip_eq(values_to_combine).zip_eq(fs_rng_vec)
            {
                combined_values += value * curr_challenge;
                let item = msm_map.entry(degree_bound).or_insert_with(|| (vec![], vec![]));
                item.0.extend_from_slice(&bases[..]);
                for scalar in scalars {
                    item.1.push((scalar * curr_challenge * randomizer).to_repr());
                }
            }

            // adjusted_witnes merged into combined_comms
            let msm_temp = msm_map.entry(Option::None).or_insert_with(|| (vec![], vec![]));
            msm_temp.0.extend_from_slice(&[-vk.vk.g, p.w]);
            msm_temp.1.extend_from_slice(&[(combined_values * randomizer).to_repr(), (*query * randomizer).to_repr()]);
            if let Some(random_v) = p.random_v {
                msm_temp.0.push(-vk.vk.gamma_g);
                msm_temp.1.push((random_v * randomizer).to_repr());
            }

            // push witnes
            combined_witnes_base.push(p.w);
            combined_witnes_scalar.push(randomizer.to_repr());
        }
        end_timer!(acc_time);

        // Calculate msm
        let acc_time = start_timer!(|| "##### combined_comms msm msm_map");
        let combined_comms = msm_map
            .into_par_iter()
            .map(|(index, val)| {
                let acc_time_1 = start_timer!(|| format!("##### combined_comms msm inner len = {:?}", val.1.len()));
                let res = ars_msm(val.0.as_slice(), val.1.as_slice());
                end_timer!(acc_time_1);
                (index, res)
            })
            .collect::<BTreeMap<Option<usize>, E::G1Projective>>();
        end_timer!(acc_time);

        let acc_time = start_timer!(|| format!("##### witness msm len = {:?}", combined_witnes_base.len()));
        let combined_witness = ars_msm(combined_witnes_base.as_slice(), combined_witnes_scalar.as_slice());
        end_timer!(acc_time);
        ...
        //     ars_msm(combined_witnes_base.as_slice(), combined_witnes_scalar.as_slice())
        // });
        // let handle3 = std::thread::spawn(move || {
        //     ars_msm(adjusted_witnes_base.as_slice(), adjusted_witnes_scalar.as_slice())
        // });
        //
        // let combined_comms = handle1.join().unwrap();
        // let combined_witness = handle2.join().unwrap();
        // let combined_adjusted_witness = handle3.join().unwrap();

        let res = Self::ars_check_elems(combined_comms, combined_witness, vk);
        return res;
    }


    /// Add by ars
    /// aggregated batch_check
    pub fn ars_batch_check_aggregated<'a>(
        vk: &VerifierKey<E>,
        commitments: Vec<(String, Vec<E::G1Affine>, Vec<E::Fr>, Option<usize>)>,
        query_set: &QuerySet<E::Fr>,
        values: &Evaluations<E::Fr>,
        proof: &BatchProof<E>,
        fs_rng: &mut S,
    ) -> Result<(BTreeMap<Option<usize>, (Vec<E::G1Affine>, Vec<<E::Fr as PrimeField>::BigInteger>)>, Vec<E::G1Affine>, Vec<<E::Fr as PrimeField>::BigInteger>), PCError>
        where
            Commitment<E>: 'a,
    {
        let commitments: BTreeMap<_, _> = commitments.into_iter().map(|c| (c.0.clone(), c)).collect();
        ...
        let res = (msm_map, combined_witnes_base, combined_witnes_scalar);
        // let res = Self::ars_check_elems(combined_comms, combined_witness, vk);
        return Ok(res);
    }

    /// Add by ars
    /// Merge the msm source data to reduce the number of calculations
    pub fn ars_check_combinations<'a>(
        vk: &VerifierKey<E>,
        linear_combinations: impl IntoIterator<Item=&'a LinearCombination<E::Fr>>,
        commitments: impl IntoIterator<Item=&'a LabeledCommitment<Commitment<E>>>,
        query_set: &QuerySet<E::Fr>,
            ...
            Commitment<E>: 'a,
    {
        ...
        let label_comm_map = commitments.into_iter().map(|c| (c.label(), c)).collect::<BTreeMap<_, _>>();
        let mut evaluations = evaluations.clone();
        let prepare_time = start_timer!(|| "## Combining commitments prepare");
        let arr = linear_combinations
            .into_iter()
            .map(|lc| {
                let lc_label = lc.label().to_string();
                let num_polys = lc.len();
                ...
                // Bases for calculating msm
                let mut comms = vec![];
                // Scalars for calculating msm
                let mut coeffs = vec![];
                ...
                for (coeff, label) in lc.iter() {
                    if label.is_one() {
                        ...
                        if num_polys == 1 && cur_comm.degree_bound().is_some() {
                            ...
                            panic!("PCError::EquationHasDegreeBounds(lc_label)")
                        }

                        // collect data
                        coeffs.push(*coeff);
                        comms.push(cur_comm.commitment().0);
                    }
                }
                (lc_label, comms, coeffs, degree_bound)
            })
            .collect::<Vec<(String, Vec<E::G1Affine>, Vec<E::Fr>, Option<usize>)>>();
        end_timer!(prepare_time);

        let instant_v = start_timer!(|| "## ars_check_combinations");
        let res = Self::ars_batch_check(vk, arr, query_set, &evaluations, proof, fs_rng);
        end_timer!(instant_v);
        return res;
    }
    ...
    /// Add by ars
    /// aggregated check_combinations
    pub fn ars_check_combinations_aggregated<'a>(
        vk: &VerifierKey<E>,
        linear_combinations: impl IntoIterator<Item=&'a LinearCombination<E::Fr>>,
        commitments: impl IntoIterator<Item=&'a LabeledCommitment<Commitment<E>>>,
        query_set: &QuerySet<E::Fr>,
            ...
            Commitment<E>: 'a,
    {
        ...
        let instant_v = start_timer!(|| "## ars_batch_check_aggregated");
        let res = Self::ars_batch_check_aggregated(vk, arr, query_set, &evaluations, proof, fs_rng);
        end_timer!(instant_v);
        return res;
    }
}
...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    fn combine_commitments<'a>(
        ...
        // Modified by ars
        return Self::ars_combine_commitments(coeffs_and_comms);
        //
    ...
    /// Add by ars
    /// Use msm
    fn ars_combine_commitments<'a>(
        coeffs_and_comms: impl IntoIterator<Item=(E::Fr, &'a Commitment<E>)>,
    ) -> E::G1Projective {
        let mut coeff_arr: Vec<<<E as PairingEngine>::Fr as PrimeField>::BigInteger> = Vec::new();
        let mut comm_arr: Vec<E::G1Affine> = Vec::new();

        for (coeff, comm) in coeffs_and_comms {
            coeff_arr.push(coeff.to_repr());
            comm_arr.push(comm.0);
        }
        ars_msm(comm_arr.as_slice(), coeff_arr.as_slice())
    }
}
...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    /// Add by ars
    /// adjusted_witness has been merged into combined_comms, Reduce adjusted_witness msm input
    #[allow(clippy::type_complexity)]
    pub fn ars_check_elems(
        combined_comms: BTreeMap<Option<usize>, E::G1Projective>,
        combined_witness: E::G1Projective,
        vk: &VerifierKey<E>,
    ) -> Result<bool, PCError> {
        let check_time = start_timer!(|| "Checking elems");
        let mut g1_projective_elems = Vec::with_capacity(combined_comms.len() + 1);
        let mut g2_prepared_elems = Vec::with_capacity(combined_comms.len() + 1);
        ...
        let check_time_1 = start_timer!(|| "## Checking elems batch_normalization_into_affine");
        let g1_prepared_elems_iter = E::G1Projective::batch_normalization_into_affine(g1_projective_elems)
            .into_iter()
            .map(|a| a.prepare())
            .collect::<Vec<_>>();
        end_timer!(check_time_1);

        let check_time_1 = start_timer!(|| "## Checking elems product_of_pairings");
        let g1_g2_prepared = g1_prepared_elems_iter.iter().zip_eq(g2_prepared_elems.iter());
        let is_one: bool = E::product_of_pairings(g1_g2_prepared).is_one();
        end_timer!(check_time_1);
...
/// Add by ars
/// Three calculation methods according to the length
pub fn ars_msm<G: AffineCurve>(bases: &[G], scalars: &[<G::ScalarField as PrimeField>::BigInteger]) -> G::Projective {
    // Determine the bucket size `c` (chosen empirically).
    let len = bases.len();
    if 1 == len {
        // let mut res = G::Projective::zero();
        let option = G::ScalarField::from_repr(scalars[0]).unwrap();
        return if option.is_one() { bases[0].to_projective() } else { bases[0].mul(option) };
    }
    if len > 1 && len <= 2 {
        return naive_variable_base_msm(bases, scalars);
    }
    return batched::msm(bases, scalars);
}
