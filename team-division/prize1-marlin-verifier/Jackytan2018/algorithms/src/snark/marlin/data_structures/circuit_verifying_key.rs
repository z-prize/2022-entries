...
// Add by ars
// Function of verifying_key prepare Each call takes a lot of time, and some values remain unchanged at runtime, so caching reduces the repeated calculation time
static ARS_GLOBAL_PREPARED_CIRCUIT_VERIFYING_KEY_ONCE: Once = Once::new();
static ARS_GLOBAL_PREPARED_INDEX_COMMS_ONCE: Once = Once::new();
lazy_static::lazy_static! {
    pub static ref ARS_GLOBAL_PREPARED_CIRCUIT_VERIFYING_KEY_CACHE: parking_lot::RwLock<Option<sonic_pc::PreparedVerifierKey<Bls12_377>>> =parking_lot::RwLock::new(None);
    pub static ref ARS_PREPARED_INDEX_COMMS_CACHE: parking_lot::RwLock<Option<Vec<sonic_pc::PreparedCommitment<Bls12_377>>>> =parking_lot::RwLock::new(None);
}
...
impl<E: PairingEngine, MM: MarlinMode> CircuitVerifyingKey<E, MM> {
    ...
    /// Add by ars
    fn ars_prepare(&self) -> PreparedCircuitVerifyingKey<E, MM> {

        // First generation, cached to global variables
        ARS_GLOBAL_PREPARED_CIRCUIT_VERIFYING_KEY_ONCE.call_once(|| {
            let mut cache_write = ARS_GLOBAL_PREPARED_CIRCUIT_VERIFYING_KEY_CACHE.write();
            let verifier_key_prepare = self.verifier_key.prepare();
            let any = &verifier_key_prepare as &dyn Any;
            let res = any.downcast_ref::<sonic_pc::PreparedVerifierKey<Bls12_377>>().unwrap();
            *cache_write = Some(res.clone());
            drop(cache_write);
        });

        // Get global variables after caching
        let cache_read = ARS_GLOBAL_PREPARED_CIRCUIT_VERIFYING_KEY_CACHE.read().clone().unwrap();
        let cache_tmp = &cache_read as &dyn Any;
        let cache_tmp = cache_tmp.downcast_ref::<sonic_pc::PreparedVerifierKey<E>>().unwrap();
        let prepared_verifier_key = cache_tmp.clone();


        // First generation, cached to global variables
        ARS_GLOBAL_PREPARED_INDEX_COMMS_ONCE.call_once(|| {
            let mut cache_write = ARS_PREPARED_INDEX_COMMS_CACHE.write();
            let mut prepared_index_comms = Vec::<sonic_pc::PreparedCommitment<E>>::new();
            for (_, comm) in self.circuit_commitments.iter().enumerate() {
                prepared_index_comms.push(comm.prepare());
            }
            let any = &prepared_index_comms as &dyn Any;
            let res = any.downcast_ref::<Vec<sonic_pc::PreparedCommitment<Bls12_377>>>().unwrap();
            *cache_write = Some(res.clone());
            drop(cache_write);
        });

        // Get global variables after caching
        let cache_read = ARS_PREPARED_INDEX_COMMS_CACHE.read().clone().unwrap();
        let cache_tmp = &cache_read as &dyn Any;
        let cache_tmp = cache_tmp.downcast_ref::<Vec<sonic_pc::PreparedCommitment<E>>>().unwrap();
        let prepared_index_comms = cache_tmp.clone();
