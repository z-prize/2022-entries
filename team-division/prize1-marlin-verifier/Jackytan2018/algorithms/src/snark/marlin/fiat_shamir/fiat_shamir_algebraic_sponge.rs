    ...
    S: DefaultCapacityAlgebraicSponge<BaseField, 6>,
{
    ...
    /// Add by ars
    fn ars_absorb_nonnative_field_elements(&mut self, elems: &[Vec<TargetField>], ty: OptimizationType) {
        Self::ars_push_elements_to_sponge(&mut self.s, elems, ty);
    }

    /// Add by ars
    fn ars_load_cache(&mut self, batch_size: usize) -> bool {
        self.s.ars_load_cache(batch_size)
    }

    /// Add by ars
    fn ars_store_cache(&mut self, batch_size: usize) -> bool {
        self.s.ars_store_cache(batch_size)
    }
}
    ...
    FiatShamirAlgebraicSpongeRng<TargetField, BaseField, S>
{
    ...
    /// Add by ars
    /// Unified calculation of limbs
    pub fn ars_push_elements_to_sponge(sponge: &mut S, src_list: &[Vec<TargetField>], ty: OptimizationType) {
        let dest_limbs_list = src_list.iter().map(|src| {
            let mut src_limbs = Vec::<(BaseField, BaseField)>::new();
            for elem in src {
                let limbs = Self::get_limbs_representations(elem, ty).unwrap();
                for limb in limbs.iter() {
            ...
            Self::compress_elements(&src_limbs, ty)
        }).collect::<Vec<Vec<BaseField>>>();
        for dest_limbs in dest_limbs_list.iter() {
            sponge.absorb(dest_limbs);
        }
    }
