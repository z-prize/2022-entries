...
use crate::polycommit::sonic_pc::ars_msm;
    ...
    MarlinSNARK<E, FS, MM, Input>
{
    ...
    fn init_sponge(
	    ...
	    // Modified by ars
        //First cache, then read cache directly
        if !sponge.ars_load_cache(batch_size) {
            ...
            sponge.ars_store_cache(batch_size);
        }

        // Modified by ars
        let instant_v = start_timer!(|| { format!("## init_sponge absorb_nonnative_field_elements len = {:?}", inputs.len()) });
        sponge.ars_absorb_nonnative_field_elements(inputs, OptimizationType::Weight);
        end_timer!(instant_v);
    ...
    /// Add by ars
    fn verify_batch_prepared_aggregated(
        circuit_verifying_key: &<Self as SNARK>::VerifyingKey,
        public_inputs: &[Vec<E::Fr>],
        proof: &<Self as SNARK>::Proof,
    ) -> Result<(BTreeMap<Option<usize>, (Vec<E::G1Affine>, Vec<<E::Fr as PrimeField>::BigInteger>)>, Vec<E::G1Affine>, Vec<<E::Fr as PrimeField>::BigInteger>), SNARKError>
    {
        ...
        let mut sponge = Self::init_sponge(batch_size, &circuit_verifying_key.circuit_commitments, &padded_public_inputs);

        // --------------------------------------------------------------------
        // First round
        Self::absorb_labeled(&first_commitments, &mut sponge);
        let (_, verifier_state) = AHPForR1CS::<_, MM>::verifier_first_round(circuit_verifying_key.circuit_info, batch_size, &mut sponge)?;
        // --------------------------------------------------------------------
        ...
        let res = SonicKZG10::<E, FS>::ars_check_combinations_aggregated(
    ...
    Input: ToConstraintField<E::Fr> + ?Sized,
{
    ...
    fn verify_batch_prepared<B: Borrow<Self::VerifierInput>>(
        ...
        // Modified by ars
        let evaluations_are_correct = SonicKZG10::<E, FS>::ars_check_combinations(
            &circuit_verifying_key.verifier_key,
            lc_s.values(),
            &commitments,
            &query_set.to_set(),
    ...
    /// Add by ars
    /// Batch aggregation operation
    fn verify_batch_aggregated<B: Borrow<Self::VerifierInput>>(
        verifying_key: &Self::VerifyingKey,
        round_proofs: &[&(Self::Proof, Vec<B>)],
    ) -> Result<bool, SNARKError> {
        if round_proofs.len() == 0 {
            return Ok(false)
        }

        let verifier_time = start_timer!(|| "Marlin::Verify");
        let list = round_proofs.iter().map(|(proof, input_list)| {
            let inputs = input_list.iter().map(|item| {
                item.borrow().to_field_elements().unwrap()
            }).collect::<Vec<_>>();
            (proof, inputs)
        }).collect::<Vec<_>>();

        let res = list.par_iter().map(|(proof, input_list)| {
            Self::verify_batch_prepared_aggregated(verifying_key, input_list.as_slice(), proof).expect("TODO: panic message")
        }).collect::<Vec<_>>();

        let mut msm_map_all = BTreeMap::new();
        let mut combined_witnes_bases_all = vec![];
        let mut combined_witnes_scalars_all = vec![];

        for (msm_map, combined_witnes_base, combined_witnes_scalar) in res {
            for (degree_bound, (bases, scalars)) in msm_map {
                let msm_map_item = msm_map_all.entry(degree_bound).or_insert_with(|| (vec![], vec![]));
                msm_map_item.0.extend_from_slice(bases.as_slice());
                msm_map_item.1.extend_from_slice(scalars.as_slice());
            }
            combined_witnes_bases_all.extend_from_slice(combined_witnes_base.as_slice());
            combined_witnes_scalars_all.extend_from_slice(combined_witnes_scalar.as_slice());
        }
        ...
        let handle2 = std::thread::spawn(move || {
            ars_msm(combined_witnes_bases_all.as_slice(), combined_witnes_scalars_all.as_slice())
        });

        let combined_witness = handle2.join().unwrap();
        let combined_comms = handle1.join().unwrap();

        // elems aggregation check
        let res = SonicKZG10::<E, FS>::ars_check_elems(combined_comms, combined_witness, &verifying_key.verifier_key)?;
        if !res {
            #[cfg(debug_assertions)]
            eprintln!("SonicKZG10::<E, FS>::Check aggregate failed");
        }
        end_timer!(verifier_time, || format!(
            " SonicKZG10::<E, FS>::Check for AHP aggregate Verifier linear equations: {}",
            res
        ));
