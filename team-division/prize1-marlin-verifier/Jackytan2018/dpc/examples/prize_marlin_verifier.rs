...
fn verify_group(
    ...
    // Index array of error proofs
    let mut faulty_positions = vec![];
    // Error proofs
    let mut faulty_proofs = vec![];
    // Index array of error proofs
    let mut right_positions = vec![];
    // Right proofs
    let mut right_proofs = vec![];

    group_proofs.iter().enumerate().for_each(|(position,(item, faulty))| {
        if *faulty {
            // faulty proof
            faulty_positions.push(position);
            faulty_proofs.push(item);
        }else {
            // right proof
            right_positions.push(position);
            right_proofs.push(item);
        }
    });

    let faulty_cnt = faulty_proofs.len();

    // faulty_cnt jobs to verify each proof of error
    // 1 jobs to aggregation verify all correctly proofs
    // so new faulty_cnt + 1 jobs
    let mut job_pool = snarkvm_utilities::ExecutionPool::with_capacity(faulty_cnt + 1);


    // Because if the aggregation proves correct, it can be inferred that each proof is correct,
    ...
    // execute all jobs
    let job_verify_results = job_pool.execute_all();

    let mut results = vec![false;group_proofs.len()];

    //Restore error results
    for (index,position) in faulty_positions.into_iter().enumerate() {
        results[position] = job_verify_results[index];
    }

    //Restore right results
    for position in right_positions {
        results[position] = job_verify_results[faulty_cnt];
    }
