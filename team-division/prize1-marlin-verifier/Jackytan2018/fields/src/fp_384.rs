...
use blst::{blst_fp, blst_fp_mul, blst_fp_sqr};
...
impl<P: Fp384Parameters> Field for Fp384<P> {
    ...
    // }
        ...
        /// Modified by ars
        /// The square of blst library is faster
        #[inline]
        fn square_in_place(&mut self) -> &mut Self {
            unsafe {
                let ctx_mut = std::mem::transmute::<&mut BigInteger, &mut blst_fp>(&mut self.0);
                blst_fp_sqr(ctx_mut, ctx_mut);
            }
            self
        }
...
/// Modified by ars
/// The mul of blst library is faster
impl<'a, P: Fp384Parameters> MulAssign<&'a Self> for Fp384<P> {
    #[inline]
    fn mul_assign(&mut self, other: &Self) {
        unsafe {
            let ctx_mut = std::mem::transmute::<&mut BigInteger, &mut blst_fp>(&mut self.0);
            let ctx_other = std::mem::transmute::<&BigInteger, &blst_fp>(&other.0);
            blst_fp_mul(ctx_mut, ctx_mut, ctx_other);
        }
    }
}
