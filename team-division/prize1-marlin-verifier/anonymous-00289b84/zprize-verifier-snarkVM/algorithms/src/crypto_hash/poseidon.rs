...
impl<F: PrimeField, const RATE: usize, const CAPACITY: usize> PoseidonSponge<F, RATE, CAPACITY> {
    ...
    fn apply_s_box(&mut self, is_full_round: bool) {
        ...
        if is_full_round {
            for elem in self.state.iter_mut() {
                ...
                tmp.square_in_place().square_in_place();
                *elem *= tmp;
            }
        }
        // Partial rounds apply the S Box (x^alpha) to just the first element of state
            ...
            let mut tmp = self.state[0].clone();
            tmp.square_in_place().square_in_place();
            self.state[0] *= tmp;
        }
    }
    ...
    fn apply_mds(&mut self) {
        ...
        let state1 = self.state[1];
        let state2 = self.state[2];
        let state3 = self.state[3];
        let state4 = self.state[4];
        let state5 = self.state[5];
        let state6 = self.state[6];

        let mds0 = &self.parameters.mds[0];
        self.state[0] = state0 * mds0[0] + state1 * mds0[1] + state2 * mds0[2] + state3 * mds0[3] + state4 * mds0[4] + state5 * mds0[5] + state6 * mds0[6]; 
        let mds1 = &self.parameters.mds[1];
        self.state[1] = state0 * mds1[0] + state1 * mds1[1] + state2 * mds1[2] + state3 * mds1[3] + state4 * mds1[4] + state5 * mds1[5] + state6 * mds1[6]; 
        let mds2 = &self.parameters.mds[2];
        self.state[2] = state0 * mds2[0] + state1 * mds2[1] + state2 * mds2[2] + state3 * mds2[3] + state4 * mds2[4] + state5 * mds2[5] + state6 * mds2[6]; 
        let mds3 = &self.parameters.mds[3];
        self.state[3] = state0 * mds3[0] + state1 * mds3[1] + state2 * mds3[2] + state3 * mds3[3] + state4 * mds3[4] + state5 * mds3[5] + state6 * mds3[6]; 
        let mds4 = &self.parameters.mds[4];
        self.state[4] = state0 * mds4[0] + state1 * mds4[1] + state2 * mds4[2] + state3 * mds4[3] + state4 * mds4[4] + state5 * mds4[5] + state6 * mds4[6]; 
        let mds5 = &self.parameters.mds[5];
        self.state[5] = state0 * mds5[0] + state1 * mds5[1] + state2 * mds5[2] + state3 * mds5[3] + state4 * mds5[4] + state5 * mds5[5] + state6 * mds5[6]; 
        let mds6 = &self.parameters.mds[6];
        self.state[6] = state0 * mds6[0] + state1 * mds6[1] + state2 * mds6[2] + state3 * mds6[3] + state4 * mds6[4] + state5 * mds6[5] + state6 * mds6[6]; 
