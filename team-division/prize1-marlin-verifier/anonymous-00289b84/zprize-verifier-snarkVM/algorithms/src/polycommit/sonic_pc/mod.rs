...
use crate::{
    ...
    snark::marlin::{params::OptimizationType, FiatShamirRng}, msm::VariableBase,
};
...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    pub fn batch_check<'a>(
        ...
        proof: &BatchProof<E>,
        fs_rng: &mut S,
        fs_parameters: Option<&S::Parameters>,
    ) -> Result<bool, PCError>
    where
        Commitment<E>: 'a,
    {
        ...
        let mut batch_kzg_check_fs_rng = match fs_parameters {
            Some(fs_parameters) => {
                S::new_with_parameters(fs_parameters)
            },
            None => {
                S::new()
            }
        ...
        Commitment<E>: 'a,
    {
        ...
        for lc in linear_combinations {
            ...
            if coeffs_and_comms.len() < 25 {
                lc_commitments.push(Self::combine_commitments(coeffs_and_comms));
            } else {
                //use msm
                let (coeffs, commits): (Vec<_>, Vec<Commitment<_>>) = coeffs_and_comms.iter().cloned().unzip();
                let scalars: Vec<_> = coeffs.iter().map(|scalar| scalar.to_repr()).collect();
                let bases: Vec<_> = commits.iter().map(|base| base.0).collect();
                let lc_commit = VariableBase::msm(&bases, &scalars);
                lc_commitments.push(lc_commit);
            }
            end_timer!(lc_time);
            lc_info.push((lc_label, degree_bound));
        }
        ...
        Self::batch_check(vk, &lc_commitments, query_set, &evaluations, proof, fs_rng, fs_parameters)
    }
}
...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    fn accumulate_elems<'a>(
        ...
        let mut tmp_combined = BTreeMap::new();
        ...
        for (labeled_comm, value) in commitments.into_iter().zip_eq(values) {
            let mut curr_challenge = fs_rng.squeeze_short_nonnative_field_element().unwrap();
            ...
            tmp_combined.entry(degree_bound).or_insert_with(|| Vec::new()).push((curr_challenge, comm));
        }

        for (degree_bound, comm_with_challenges) in tmp_combined.into_iter() {
            let comm_with_challenge = if comm_with_challenges.len() < 25 {
                Self::combine_commitments(comm_with_challenges)
            } else {
                let (challenges, commits): (Vec<_>, Vec<Commitment<_>>) = comm_with_challenges.iter().cloned().unzip();
                let scalars: Vec<_>= challenges.iter().map(|scalar| scalar.to_repr()).collect();
                let bases: Vec<_> = commits.iter().map(|base| base.0).collect();
