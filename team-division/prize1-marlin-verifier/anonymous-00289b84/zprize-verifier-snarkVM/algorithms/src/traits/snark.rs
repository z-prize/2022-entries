...
use crate::{errors::SNARKError, AlgebraicSponge, snark::marlin::FiatShamirRng};
...
pub trait SNARK {
    ...
    //type FiatShamirRng: FiatShamirRng<Self::ScalarField, Self::BaseField>;
    type FSParameters;
    ...
    fn verify_batch_prepared<B: Borrow<Self::VerifierInput>>(
        fs_parameters: Option<&Self::FSParameters>,
        prepared_verifying_key: &<Self::VerifyingKey as Prepare>::Prepared,
        input: &[B],
        proof: &Self::Proof,
    ) -> Result<bool, SNARKError>;

    fn verify_batch_with_prepared_vk<B: Borrow<Self::VerifierInput>>(
        fs_parameters: &Self::FSParameters,
        prepared_vk: &<Self::VerifyingKey as Prepare>::Prepared,
        input: &[B],
        proof: &Self::Proof,
    ) -> Result<bool, SNARKError> {
        Self::verify_batch_prepared(Some(fs_parameters), prepared_vk, input, proof)
    }
    ...
    fn verify<B: Borrow<Self::VerifierInput>>(
        ...
        Self::verify_batch(None, verifying_key, &[input], proof)
    }
}
