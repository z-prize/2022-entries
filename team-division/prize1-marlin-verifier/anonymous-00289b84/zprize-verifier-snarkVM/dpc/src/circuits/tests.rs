...
fn dpc_execute_circuits_test<N: Network>(
    ...
    assert!(
        <N as Network>::InputSNARK::verify_batch(None, &input_verifying_key, &input_public_inputs, &input_proof).unwrap()
    );
    ...
    // Verify that the inner circuit proof passes.
    assert!(
        <N as Network>::OutputSNARK::verify_batch(None, &output_verifying_key, &output_public_inputs, &output_proof).unwrap()
    );
    println!("Verifying time: {}", verifying_start.elapsed().as_secs_f64());
