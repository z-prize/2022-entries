...
impl<P: Fp384Parameters> Field for Fp384<P> {
    ...
    fn square_in_place(&mut self) -> &mut Self {
        ...
        #[cfg(feature = "blst_asm")]
        {
            let mut res: vec384 = [0u64; 6];
            unsafe {
                let a_ptr = self.0.0.as_ptr() as *const limb_t;
                blst_fp_sqr(res.as_mut_ptr() as *mut limb_t, a_ptr, P::MODULUS.0.as_ptr() as *const limb_t, P::INV);
            }
            std::mem::swap(&mut self.0.0, &mut res);
            self
        }
    }
    ...
    fn inverse(&self) -> Option<Self> {
        ...
        #[cfg(feature = "blst_asm")]
        {
            if self.is_zero() {
                ...
                unsafe {
                    let a_ptr = self.0.0.as_ptr() as *const limb_t;
                    blst_fp_eucl_inverse(res.0.0.as_mut_ptr() as *mut limb_t, a_ptr, P::MODULUS.0.as_ptr() as *const limb_t, P::INV);
                }
                Some(res)
            }
        }
...
impl<'a, P: Fp384Parameters> MulAssign<&'a Self> for Fp384<P> {
    ...
    fn mul_assign(&mut self, other: &Self) {
        ...
        #[cfg(feature = "blst_asm")]
        {
            ...
            unsafe {
                let a_ptr = self.0.0.as_ptr() as *const limb_t;
                let b_ptr = other.0.0.as_ptr() as *const limb_t;
                blst_fp_mul(res.as_mut_ptr() as *mut limb_t, a_ptr, b_ptr, P::MODULUS.0.as_ptr() as *const limb_t, P::INV);
            }
            std::mem::swap(&mut self.0.0, &mut res);
        }
    }
