...
impl<F: PrimeField, const RATE: usize, const CAPACITY: usize> PoseidonSponge<F, RATE, CAPACITY> {
    #[inline]
    fn apply_ark_opt(&mut self, round_number: usize, is_full_round: bool) {
        if is_full_round {
            for (state_elem, ark_elem) in self.state.iter_mut().zip(&self.parameters.ark_opt[round_number]) {
                *state_elem += ark_elem;
            }
        }
        else {
            self.state[0] += &self.parameters.ark_opt[round_number][0];
        }
    }
    ...
    #[inline]
    fn apply_mds_opt(&mut self, round_number: usize) {
        let mut new_state = State::default();

        new_state[0] = self.parameters.mds_m_0_0 * self.state[0];
        for (i, w) in self.parameters.mds_w_hat[round_number].iter().enumerate() {
            new_state[0] += *w * self.state[i + 1];
        }

        for (i, v) in self.parameters.mds_v[round_number].iter().enumerate() {
            new_state[i + 1] = (*v * self.state[0]) + self.state[i + 1];
        }
        self.state = new_state;
    }
    ...
    fn permute(&mut self) {
        ...
        for _ in 0..(full_rounds_over_2) {
            self.apply_ark_opt(rc_counter, true);
        ...
        let mut new_state = State::default();
        for row in 0..(RATE + CAPACITY) {
            for col in 0..(RATE + CAPACITY) {
                new_state[col] += self.state[row] * self.parameters.mds_m_i[row][col];
            }
        }
        self.state = new_state;
        ...
        for r in 0..(partial_rounds) {
            ...
            if r < (partial_rounds - 1) {
                rc_counter += 1;
                self.apply_ark_opt(rc_counter, false);
            }
            self.apply_mds_opt(partial_rounds - r - 1);
        }
        rc_counter += 1;
