...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
        ...
        Commitment<E>: 'a,
    {
        ...
        // Make randomization deterministic based on w
        let w_seed = proof.0[0].w.to_x_coordinate().to_repr_unchecked().to_bytes_le().unwrap();
        let mut rng = ChaCha8Rng::seed_from_u64(w_seed[0] as u64);
        ...
        for ((_query_name, (query, labels)), p) in query_to_labels_map.into_iter().zip_eq(&proof.0) {
            ...
            randomizer = E::Fr::from(rng.gen::<u128>());
        }
        ...
        for (labeled_comm, value) in commitments.into_iter().zip_eq(values) {
            ...
            if degree_bound.is_some() {
                ...
                *combined_comms.entry(degree_bound).or_insert_with(E::G1Projective::zero) += &comm_with_challenge;
                ...
                coeffs_and_comms.push((curr_challenge, comm));
            }
        }

        let nc = Self::combine_commitments(coeffs_and_comms);
        let db: Option<usize> = None;
        *combined_comms.entry(db).or_insert_with(E::G1Projective::zero) += &nc;
