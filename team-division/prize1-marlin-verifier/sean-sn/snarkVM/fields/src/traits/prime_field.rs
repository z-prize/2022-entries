    ...
    FftField<FftParameters = <Self as PrimeField>::Parameters> + PoseidonDefaultField + FromStr<Err = FieldError>
{
    ...
    fn from_repr_raw(_repr: Self::BigInteger) -> Self {
        // Put something here so we don't have to add it everywhere
        Self::from_bytes_be_mod_order(&[0])
    }

    fn get_ark_consts() -> Vec<Vec<Self>> {
        let ark = Vec::<Vec<Self>>::new();
        ark
    }

    fn get_mds_consts() -> Vec<Vec<Self>> {
        let mds = Vec::<Vec<Self>>::new();
        mds
    }
    ...
    fn get_ark_opt_consts() -> Vec<Vec<Self>> {
        let ark = Vec::<Vec<Self>>::new();
        ark
    }
