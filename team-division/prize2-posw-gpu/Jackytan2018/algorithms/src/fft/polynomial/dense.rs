...
use snarkvm_utilities::{cfg_iter_mut, cfg_chunks_mut, serialize::*};
...
impl<F: Field> DensePolynomial<F> {
    ...
    pub fn ars_evaluate_powers_of_point(&self, point: F) -> Vec<F> {
        if self.is_zero() {
            return vec![F::zero()];
        } else if point.is_zero() {
        ...
        let mut powers_of_point = vec![F::one(); self.degree()+1];
        let num_elem_per_thread = 64;
        cfg_chunks_mut!(powers_of_point, num_elem_per_thread).enumerate().for_each(|(i, chunk)| {
            let _offset = i * num_elem_per_thread;
            let mut offset_pow = point.pow([(i * num_elem_per_thread) as u64]);
            chunk.iter_mut().enumerate().for_each(|(_t,coeff)| {
                *coeff = offset_pow;
                offset_pow *= &point
            })
        });
    ...
    pub fn ars_evaluate(&self, point: F, powers_of_point:&Vec<F>) -> F {
        if self.is_zero() {
            return F::zero();
        } else if point.is_zero() {
            return self.coeffs[0];
