...
impl<'a, F: Field> Polynomial<'a, F> {
    ...
    pub fn divide_with_q_and_r(&self, divisor: &Self) -> Option<(DensePolynomial<F>, DensePolynomial<F>)> {
        if self.is_zero() {
            ...
            let _divisor_leading_inv = F::one();
            ...
            if let Sparse(p) = divisor {
                while !remainder.is_zero() && remainder.degree() >= divisor.degree() {
                    ...
                    let mut t = 0;
                    for (i, _div_coeff) in p.coeffs() {
                        if t == 0 {
                            remainder[cur_q_degree + i] += &(cur_q_coeff);
                            t = 1;
                        }else{
                            remainder[cur_q_degree + i] -= &(cur_q_coeff);
                        }
    ...
    pub fn ars_evaluate_powers_of_point(&self, point: F) -> Vec<F> {
        match self {
            Sparse(p) => p.ars_evaluate_powers_of_point(point),
            Dense(p) => p.ars_evaluate_powers_of_point(point),
        }
    }
    #[inline]
    pub fn ars_evaluate(&self, point: F, powers_of_point:&Vec<F>) -> F {
        match self {
            Sparse(p) => p.ars_evaluate(point, powers_of_point),
            Dense(p) => p.ars_evaluate(point, powers_of_point),
        }
    }    
}
