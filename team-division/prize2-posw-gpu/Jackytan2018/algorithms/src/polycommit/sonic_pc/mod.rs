...
use snarkvm_fields::{One, Zero, PrimeField};
...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    where
        Commitment<E>: 'a,
    {
        return  Self::ars_check_combinations(vk, linear_combinations, commitments, query_set, evaluations, proof, fs_rng);        
    }
        ...
        where Commitment<E>: 'a,
    {
        ...
        let mut batch_kzg_check_fs_rng = S::new();
        batch_kzg_check_fs_rng.absorb_nonnative_field_elements(query_set.iter().map(|(_, (_, q))| *q), OptimizationType::Weight);
        batch_kzg_check_fs_rng.absorb_nonnative_field_elements(values.values().copied(), OptimizationType::Weight);
        proof.absorb_into_sponge(&mut batch_kzg_check_fs_rng)?;
        ...
        for v in query_to_labels_map.into_iter() {
            ...
            if cnt == 0 {
                ...
                randomizer_list.push(batch_kzg_check_fs_rng.squeeze_short_nonnative_field_element().unwrap());
            }
            cnt += 1;
        }

        let res_list = query_to_labels_list.into_par_iter().zip_eq(&proof.0).zip_eq(randomizer_list).zip_eq(fs_rng_list).map(|((((_query_name, (query, labels)), p), randomizer), fs_rng_vec)| {
            let mut comms_to_combine: Vec<&'_ LabeledCommitment<_>> = Vec::new();
            let mut values_to_combine = Vec::new();

            for label in labels.into_iter() {
                let commitment = commitments.get(label).ok_or(PCError::MissingPolynomial { label: label.to_string() }).unwrap();
                let v_i = values.get(&(label.clone(), *query)).ok_or(PCError::MissingEvaluation { label: label.to_string() }).unwrap();
                comms_to_combine.push(commitment);
                values_to_combine.push(*v_i);
            }

            Self::ars_accumulate_elems(
                vk,
                comms_to_combine.into_iter(),
                *query,
                values_to_combine.into_iter(),
        ...
        for (combined_comm, combined_witnes, combined_adjusted_witnes) in res_list {
            for (key, value, _) in combined_comm.into_iter() {
                *combined_comms.entry(key).or_insert_with(E::G1Projective::zero) += value;
            }
            combined_witness += combined_witnes;
            combined_adjusted_witness += combined_adjusted_witnes;
        }
        ...
        where Commitment<E>: 'a,
    {
        ...
        let mut coeffs_and_comms_list = vec![];
        ...
        for lc in linear_combinations {
            ...
            coeffs_and_comms_list.push(coeffs_and_comms);
            lc_info.push((lc_label, degree_bound));
        }

        let lc_commitments = coeffs_and_comms_list.into_par_iter().map(|coeffs_and_comms| {
            let res = Self::combine_commitments(coeffs_and_comms);
            res
        }).collect();
        ...
        Self::ars_batch_check(vk, &lc_commitments, query_set, &evaluations, proof, fs_rng)
    }

    pub fn ars_open_combinations<'a>(
        ck: &CommitterKey<E>,
        linear_combinations: impl IntoIterator<Item = &'a LinearCombination<E::Fr>>,
        polynomials: impl IntoIterator<Item = &'a LabeledPolynomial<E::Fr>>,
        commitments: impl IntoIterator<Item = &'a LabeledCommitment<Commitment<E>>>,
        ...
        Commitment<E>: 'a,
    {
        ...
        for (_point_name, (&query, labels)) in query_to_labels_map.into_iter() {
            ...
            pool.add_job(move || {
                let proof = kzg10::KZG10::ars_open(&ck.powers(), &polynomial, query, &rand);
                proof
            });
        }
        let batch_proof = pool.execute_all().into_iter().collect::<Result<_, _>>().map(BatchProof);
...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    ) -> E::G1Projective
    {
        ...
        if coeff_arr.len() == 1 {
            ...
            let option = E::Fr::from_repr(coeff_arr[0]).unwrap();
            if option.is_one() {
                combined_comm.add_assign_mixed(&comm_arr[0]);
            } else {
                combined_comm += &comm_arr[0].mul(option);
            }
            return combined_comm;
        }

        batched::msm(comm_arr.as_slice(), coeff_arr.as_slice())
    }
    ...
    pub fn ars_commit<'b>(
        ...
        Self::ars_commit_with_terminator(ck, polynomials, &AtomicBool::new(false), rng, round)
    }
    #[allow(clippy::type_complexity)]
    #[allow(clippy::format_push_string)]
    pub fn ars_commit_with_terminator<'a>(
        ck: &CommitterKey<E>,
        polynomials: impl IntoIterator<Item = LabeledPolynomialWithBasis<'a, E::Fr>>,
        terminator: &AtomicBool,
        rng: Option<&mut dyn RngCore>,
                    ...
                    .map(move |p| {
                        ...
                        match p {
                            ...
                            PolynomialWithBasis::Monomial { polynomial, degree_bound } => {
                                ...
                                kzg10::KZG10::ars_commit(&powers, &polynomial, hiding_bound, terminator, rng_ref, round)
                            }
                        }
                    })
                    .collect::<Result<Vec<_>, _>>()?
...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    fn ars_accumulate_elems<'a>(
        ...
        proof: &kzg10::Proof<E>,
        randomizer: Option<E::Fr>,
        fs_rng_vec: Vec<<E as PairingEngine>::Fr>,
    ) -> (Vec<(Option<usize>, <E as PairingEngine>::G1Projective, <E as PairingEngine>::Fr)>, <E as PairingEngine>::G1Projective, <E as PairingEngine>::G1Projective)
    {
        let mut combined_values = E::Fr::zero();
        let commitments_list: Vec<&'a LabeledCommitment<Commitment<E>>> = commitments.into_iter().collect();
        let value_list: Vec<E::Fr> = values.into_iter().collect();

        let res = commitments_list.into_par_iter().zip_eq(value_list).zip_eq(fs_rng_vec).map(|((labeled_comm, value), curr_challenge)| {
            let mut curr_challenge = curr_challenge;
            let mut combined_value = E::Fr::zero();
            combined_value += &(value * curr_challenge);
            let comm = labeled_comm.commitment();
            let degree_bound = labeled_comm.degree_bound();

            if let Some(randomizer) = randomizer {
                curr_challenge = curr_challenge * randomizer;
            }

            let comm_with_challenge: E::G1Projective = comm.0.mul(curr_challenge);            
            (degree_bound, comm_with_challenge, combined_value)
        }).collect::<Vec<_>>();

        for (_, _, c) in res.iter() {
            combined_values += c;
        }
        ...
        let (witnes, adjusted_witnes) = if let Some(randomizer) = randomizer {
            (proof.w.mul(randomizer), adjusted_witness.mul(randomizer))
        } else {
            (proof.w.to_projective(), adjusted_witness)
        };

        (res, witnes, adjusted_witnes)
    }
}
