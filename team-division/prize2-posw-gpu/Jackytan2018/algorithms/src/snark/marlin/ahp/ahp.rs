...
use crate::fft::cuda_fft::{domain_elements_gpu, compute_deri_vanishing_poly};
...
impl<F: PrimeField, MM: MarlinMode> AHPForR1CS<F, MM> {
    ...
    #[allow(non_snake_case)]
    pub fn ars_construct_linear_combinations<E: EvaluationsProvider<F>>(
        public_inputs: &[Vec<F>],
        evals: &E,
        prover_third_message: &prover::ThirdMessage<F>,
        state: &verifier::State<F, MM>,
    ) -> Result<(BTreeMap<String, LinearCombination<F>>, BTreeMap<String, F>), AHPError> {
        assert!(!public_inputs.is_empty());
        let constraint_domain = state.constraint_domain;
        let non_zero_a_domain = state.non_zero_a_domain;
        let non_zero_b_domain = state.non_zero_b_domain;
        ...
        let mut lc_key_list: Vec<String> = vec!("g_1".to_string(),"g_a".to_string(),"g_b".to_string(),"g_c".to_string());
        let mut point_list: Vec<&F> = vec!(&beta, &gamma, &gamma, &gamma);
        ...
        let z_b_s = (0..state.batch_size)
            .map(|i| {
                let z_b_i = witness_label("z_b", i);
                let z_b_i_str = z_b_i.clone();
                lc_key_list.push(z_b_i_str);
                point_list.push(&beta);
                LinearCombination::new(z_b_i.clone(), [(F::one(), z_b_i)])
            })
            .collect::<Vec<_>>();

        let lc_eval_list = evals.ars_save_lc_eval_list(lc_key_list, &point_list);
        ...
        let z_b_s_at_beta = z_b_s.iter().map(|z_b| evals.ars_get_lc_eval(&lc_eval_list, z_b.label.clone())).collect::<Result<Vec<_>, _>>()?;
        let batch_z_b_at_beta: F =
            z_b_s_at_beta.iter().zip_eq(batch_combiners).map(|(z_b_at_beta, combiner)| *z_b_at_beta * combiner).sum();

        let g_1_at_beta = evals.ars_get_lc_eval(&lc_eval_list, "g_1".to_string())?;
        ...
        let g_a = LinearCombination::new("g_a", [(F::one(), "g_a")]);
        let g_a_at_gamma = evals.ars_get_lc_eval(&lc_eval_list, "g_a".to_string())?;
        ...
        let g_b = LinearCombination::new("g_b", [(F::one(), "g_b")]);
        let g_b_at_gamma = evals.ars_get_lc_eval(&lc_eval_list, "g_b".to_string())?;
        let selector_b = largest_non_zero_domain.evaluate_selector_polynomial(non_zero_b_domain, gamma);
        let lhs_b =
            Self::construct_lhs("b", alpha, beta, gamma, v_H_at_alpha * v_H_at_beta, g_b_at_gamma, *sum_b, selector_b);
        matrix_sumcheck += (r_b, &lhs_b);
        ...
        let g_c = LinearCombination::new("g_c", [(F::one(), "g_c")]);
          let g_c_at_gamma = evals.ars_get_lc_eval(&lc_eval_list, "g_c".to_string())?;
        let selector_c = largest_non_zero_domain.evaluate_selector_polynomial(non_zero_c_domain, gamma);
        let lhs_c =
            Self::construct_lhs("c", alpha, beta, gamma, v_H_at_alpha * v_H_at_beta, g_c_at_gamma, *sum_c, selector_c);
        matrix_sumcheck += (r_c, &lhs_c);
        ...
        Ok((linear_combinations, lc_eval_list))
    }    
}
...
pub trait EvaluationsProvider<F: PrimeField>: core::fmt::Debug {
    ...
    fn ars_save_lc_eval_list(&self, lc_key_list: Vec<String>, point_list: &Vec<&F>) -> BTreeMap<String, F>;
    fn ars_get_lc_eval(&self,lc_eval_list: &BTreeMap<String, F>, lc_key: String) -> Result<F, AHPError>;    
}
...
impl<'a, F: PrimeField> EvaluationsProvider<F> for crate::polycommit::sonic_pc::Evaluations<'a, F> {
    ...
    fn ars_save_lc_eval_list(&self, lc_key_list: Vec<String>, point_list: &Vec<&F>) -> BTreeMap<String, F> {
        let mut lc_eval_list = BTreeMap::new();
        for (k, v) in lc_key_list.iter().enumerate(){
            let key = (lc_key_list[k].clone(), *point_list[k]);
            if let Some(value) = self.get(&key) {
                lc_eval_list.insert(v.to_string(), *value);
            }
        }
        lc_eval_list
    }
    fn ars_get_lc_eval(&self,lc_eval_list: &BTreeMap<String, F>, lc_key: String) -> Result<F, AHPError> {
        let lc_eval = F::one();
        if let Some(value) = lc_eval_list.get(&lc_key){
            return Ok(*value);
        }
        Ok(lc_eval)
    }    
    ...
    T: Borrow<LabeledPolynomial<F>> + core::fmt::Debug,
{
    ...
    fn ars_save_lc_eval_list(&self, lc_key_list: Vec<String>, point_list: &Vec<&F>) -> BTreeMap<String, F> {
        let mut lc_eval_list = BTreeMap::new();
        let mut powers_of_point_list: BTreeMap<&F, (usize, Vec<F>)> = BTreeMap::new();

        for p in self.iter(){
            let n_degree = p.borrow().degree();
            for (k, v) in lc_key_list.iter().enumerate(){
                if p.borrow().label() == *v {
                    if let Some((o_degree, o_powers)) = &powers_of_point_list.get(point_list[k]) {
                        if n_degree > *o_degree{
                            let mut n_powers: Vec<F> = Vec::from(&o_powers[0..]);
                            let mut cur = n_powers[n_powers.len()-1]*point_list[k];
                            for _ in 0..(n_degree-o_degree) {
                                n_powers.push(cur);
                                cur *= point_list[k];
                            }
                            lc_eval_list.insert(v.to_string(), p.borrow().ars_evaluate(*point_list[k], &n_powers));
                            powers_of_point_list.insert(point_list[k], (n_degree, n_powers));
                        }else if n_degree < *o_degree{
                            lc_eval_list.insert(v.to_string(), p.borrow().ars_evaluate(*point_list[k], &Vec::from(&o_powers[0..(n_degree+1)])));
                        }else{
                            lc_eval_list.insert(v.to_string(), p.borrow().ars_evaluate(*point_list[k], &o_powers));
                        }
                        continue;
                    }else{
                        let powers_of_point = p.borrow().ars_evaluate_powers_of_point(*point_list[k]);
                        lc_eval_list.insert(v.to_string(), p.borrow().ars_evaluate(*point_list[k], &powers_of_point));
                        powers_of_point_list.insert(point_list[k], (n_degree, powers_of_point));
                        continue;
                    }
                }
            }
...
pub trait UnnormalizedBivariateLagrangePoly<F: PrimeField> {
    ...
    fn ars_batch_eval_unnormalized_bivariate_lagrange_poly_with_diff_inputs_over_domain_gpu(
        &self,
        x: F,
        domain: &EvaluationDomain<F>,
...
impl<F: PrimeField> UnnormalizedBivariateLagrangePoly<F> for EvaluationDomain<F> {
    ...
    fn batch_eval_unnormalized_bivariate_lagrange_poly_with_diff_inputs_over_domain(
        ...
        let mut elements=vec![F::zero(); domain.size()];
        domain_elements_gpu(domain,&mut elements);
    ...
    fn ars_batch_eval_unnormalized_bivariate_lagrange_poly_with_diff_inputs_over_domain_gpu(
        ...
        let vanish_x = self.evaluate_vanishing_polynomial(x);
        let mut elements = vec![F::zero(); domain.size()];
        let ratio0 = fft_precomputation.domain.size() / domain.size();
        for i in 0..domain.size()/2 {
            elements[i] = fft_precomputation.roots[i*ratio0];
            elements[i+domain.size()/2] =- fft_precomputation.roots[i*ratio0];
        }
        ...
        if domain.size() <= self.size() {
            ...
            let ratio = domain.size() / self.size();
            let mut numerators = vec![vanish_x; domain.size()];
            let mut ratio_evaluate= vec![F::zero();  ratio];
            for i in 0..ratio {
                ratio_evaluate[i] = self.evaluate_vanishing_polynomial(elements[i]);
            }
            cfg_iter_mut!(numerators).enumerate().for_each(|(i, n)| {
                if i % ratio != 0 {
                    *n -= ratio_evaluate[i % ratio];
                }
            });
