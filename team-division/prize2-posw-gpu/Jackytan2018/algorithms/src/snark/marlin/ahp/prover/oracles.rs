...
impl<'a, F: PrimeField> FirstOracles<'a, F> {
    ...
    #[allow(clippy::needless_collect)]
    pub fn ars_iter_for_commit(&mut self) -> impl Iterator<Item = LabeledPolynomialWithBasis<'a, F>> {
        let t = self.batches.iter_mut().flat_map(|b| b.ars_iter_for_open()).collect::<Vec<_>>();
        t.into_iter().chain(self.mask_poly.clone().map(Into::into))
    }
}
...
impl<'a, F: PrimeField> SingleEntry<'a, F> {
    ...
    pub fn ars_iter_for_open(&self) -> impl Iterator<Item = LabeledPolynomialWithBasis<'a, F>> {
        let w_poly = self.w_poly.clone();
        let z_a_poly = self.z_a_poly.clone();
        let z_b_poly = self.z_b_poly.clone();
        [(w_poly.into()), z_a_poly.into(), z_b_poly.into()].into_iter()
    }
}
