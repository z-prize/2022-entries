...
impl<F: PrimeField, MM: MarlinMode> AHPForR1CS<F, MM> {
    ...
    pub fn prover_second_round<'a, R: RngCore>(
        ...
        let mul_domain_size = 4 * constraint_domain.size();
        let mul_domain = EvaluationDomain::new(mul_domain_size).expect("field is not smooth enough to construct domain");
        let fft_precomputation = &state.index.fft_precomputation;
        
        let mut r_alpha_x_evals = constraint_domain
            .ars_batch_eval_unnormalized_bivariate_lagrange_poly_with_diff_inputs_over_domain_gpu(*alpha, &mul_domain, fft_precomputation);

        let mut r_alpha_x_evals_t= vec![F::zero(); constraint_domain.size()];        
        for i in 0..constraint_domain.size() {
            r_alpha_x_evals_t[i] = r_alpha_x_evals[4*i];            
        }
        
        let fft_precomputation = &state.index.fft_precomputation;

        let mut pool = ExecutionPool::with_capacity(2);        

        pool.add_job(|| {
            let mut summed_z_m = Self::ars_calculate_summed_z_m(&state, *alpha, *eta_b, *eta_c, batch_combiners);
            summed_z_m            
        });
        pool.add_job(|| {
            let r_alpha_x_evals_t =
            constraint_domain.ars_batch_eval_unnormalized_bivariate_lagrange_poly_with_diff_inputs_over_domain_gpu(*alpha, &constraint_domain, fft_precomputation);

            let mut t = Self::ars_calculate_t(
                &[&state.index.a, &state.index.b, &state.index.c],
                [F::one(), *eta_b, *eta_c],
                state.input_domain,
                state.constraint_domain,
        ...
        let [mut summed_z_m, mut t] : [DensePolynomial<F>; 2] = pool.execute_all().try_into().unwrap();
     
        let mut z = cfg_iter!(state.first_round_oracles.as_ref().unwrap().batches)
            .zip_eq(batch_combiners)
            .zip(&state.x_poly)
            .map(|((b, &coeff), x_poly)| {
        ...
        let sumcheck_lhs = Self::ars_calculate_lhs(&state, &mut t, &mut summed_z_m, &mut z, &mut r_alpha_x_evals, *alpha);
    ...
    /// Optimization polynomial calculation
    fn ars_calculate_t<'a>(
        matrices: &[&'a Matrix<F>],
        matrix_randomizers: [F; 3],
        input_domain: EvaluationDomain<F>,
        constraint_domain: EvaluationDomain<F>,
        ...
        let mut temp_index = vec![0; constraint_domain.size()];
        for k in 0..constraint_domain.size() {
            temp_index[k]= constraint_domain.reindex_by_subdomain(input_domain, k);
        }

        for (matrix, eta) in matrices.iter().zip_eq(matrix_randomizers) {
            for (r, row) in matrix.iter().enumerate() {                
                for (coeff, c) in row.iter() {
                    let index =  temp_index[*c];
                    if eta == F::one() {
                        if coeff == &F::one() {
                            t_evals_on_h[index] += &(r_alpha_x_on_h[r]);
                        ...
                        if coeff == &F::one() {
                            t_evals_on_h[index] += &(eta  * r_alpha_x_on_h[r]);
                        }else if coeff == &F::zero() {
                            continue;
                        }else {
                            t_evals_on_h[index] += &(eta * coeff * r_alpha_x_on_h[r])
    ...
    fn ars_calculate_lhs(
        state: &prover::State<F, MM>,
        t: &mut DensePolynomial<F>,
        summed_z_m: &mut DensePolynomial<F>,
        z: &mut DensePolynomial<F>,
        r_alpha_x_evals: &mut Vec<F>,
        alpha: F,
    ) -> DensePolynomial<F> {
        let constraint_domain = state.constraint_domain;
        ...
        let mul_domain: EvaluationDomain<F> = EvaluationDomain::new(mul_domain_size).expect("field is not smooth enough to construct domain");
    
        summed_z_m.coeffs.resize(mul_domain.size(), F::zero());
        z.coeffs.resize(mul_domain.size(), F::zero());
        t.coeffs.resize(mul_domain.size(), F::zero());
        
        let mut result = r_alpha_x_evals;
        result.resize(mul_domain.size(), F::zero());

        calculate_lhs_gpu(&mut result, &summed_z_m.coeffs, &z.coeffs, &t.coeffs, &mul_domain.group_gen, &mul_domain.group_gen_inv, mul_domain.log_size_of_group, &mul_domain.size_inv, &state.fft_precomputation().roots, state.fft_precomputation().domain.size, &state.ifft_precomputation().inverse_roots, state.ifft_precomputation().domain.size).unwrap();

        let mut lhs = DensePolynomial::from_coefficients_vec(result.to_vec());
        lhs += &mask_poly.map_or(SparsePolynomial::zero(), |p| p.polynomial().as_sparse().unwrap().clone());
        lhs
    } 

    fn ars_calculate_summed_z_m(
        state: &prover::State<F, MM>,
        alpha: F,
        eta_b: F,
        eta_c: F,     
        ...
        let first_msg = state.first_round_oracles.as_ref().unwrap();        
        let eta_b_over_eta_c = eta_b * eta_c.inverse().unwrap();        
        let summed_z_m = cfg_iter!(first_msg.batches)
            .zip_eq(batch_combiners)
            .map(|(entry, combiner)| {
                let z_a = entry.z_a_poly.polynomial().as_dense().unwrap();
                let mut z_b = entry.z_b_poly.polynomial().as_dense().unwrap().clone();                   
                ...
                let mut summed_z_m = {
                    ...
                    cfg_iter_mut!(z_b.coeffs).for_each(|b| *b *= eta_c);
                    z_b.coeffs[0] += F::one();
                    let degree = z_a.degree() + z_b.degree()+2;
                    let domain: EvaluationDomain<F> = EvaluationDomain::new(degree).unwrap();
                    let mut p_a = z_a.coeffs.clone();
                    let mut p_b = z_b.coeffs.clone();
                    p_a.resize(domain.size(), F::zero());
                    p_b.resize(domain.size(), F::zero());                    

                    calculate_summed_z_m_gpu(&mut p_a, &mut p_b, &domain.group_gen, &domain.group_gen_inv, domain.log_size_of_group, &domain.size_inv, 
                        &state.fft_precomputation().roots, state.fft_precomputation().domain.size, &state.ifft_precomputation().inverse_roots,state.ifft_precomputation().domain.size).unwrap();
                    // Start undoing in place mutation, by first subtracting the 1 that we added...
                    z_b.coeffs[0] -= F::one();
                    DensePolynomial::from_coefficients_vec(p_a)
                };
                // ... and then multiplying by eta_b/eta_c, instead of just eta_b.
                cfg_iter_mut!(summed_z_m.coeffs).zip(&z_b.coeffs).for_each(|(c, b)| *c += eta_b_over_eta_c * b);
