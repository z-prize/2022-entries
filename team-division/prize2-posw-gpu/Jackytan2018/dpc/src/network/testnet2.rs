...
static ARS_PROOF_PARALLEL_ONCE: Once = Once::new();
lazy_static::lazy_static!{
    pub static ref FORK_PID: Mutex<i32> = Mutex::new(-1);
}
...
impl Network for Testnet2 {
    ...
    fn genesis_block() -> &'static Block<Self> {
        ...
        ARS_PROOF_PARALLEL_ONCE.call_once(||{
            ...
            for _ in 0..process_num{
                match fork() {
                    ...
                    Ok(Fork::Child) => {
                        *FORK_PID.lock().unwrap() = 0;
                        println!("I'm a new child process");
                        
                        rayon::ThreadPoolBuilder::new()
                            .stack_size(8 * 1024 * 1024)
                            .num_threads(CONFIG.num_rayon_cores_global)
                            .build_global()
                            .unwrap();
                        break; 
                    },
