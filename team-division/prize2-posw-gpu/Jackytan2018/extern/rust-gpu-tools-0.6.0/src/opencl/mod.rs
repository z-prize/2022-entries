...
impl Program {
    ...
    pub fn recovery_buffer_pool<T>(&self, _gpu_index: usize, _length: usize, _buffer: Buffer<T>) -> GPUResult<()> {
        Ok(())
    }
    ...
    /// This function isn't actually unsafe, it's marked as `unsafe` due to the CUDA version of it,
    /// where it is unsafe. This is done to have symmetry between both APIs.
    pub fn create_buffer_pool<T>(&self, _gpu_index: usize, length: usize) -> GPUResult<Buffer<T>> {
        assert!(length > 0);
