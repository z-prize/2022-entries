...
use ec_gpu_common::{DeviceMemory, GPU_OP};
...
impl VariableBase {
    ...
    pub fn trigger_reload_base<G: AffineCurve>() {
        let bases = vec![G::zero(); 1];
        let scalars = vec![G::ScalarField::zero().to_repr(); 1];

        let _ = gpu::msm_gpu(&bases[..], &scalars[..], 0, GPU_OP::RESET, false);
    }

    pub fn msm_gpu_pippenger<G: AffineCurve>(
        bases: &[G],
        scalars: &[<G::ScalarField as PrimeField>::BigInteger],
    ) -> G::Projective {
        gpu::msm_gpu(bases, scalars, 0, GPU_OP::LOAD_BASE, false).unwrap()
    }

    pub fn msm_gpu_pippenger_scalar<G: AffineCurve, S>(
        bases: &[G],
        scalars: &[S],
        is_mont_form: bool,
    ) -> G::Projective {
        gpu::msm_gpu(bases, scalars, 0, GPU_OP::LOAD_BASE, is_mont_form).unwrap()
    }

    pub fn msm_gpu_ptr_reuse_base<G: AffineCurve, Fg>(
        bases: &[G],
        ...
        gpu::msm_gpu_ptr(bases, scalars, start, gpu_op, is_mont_form).unwrap()
    }

    pub fn msm_gpu_reuse_base<G: AffineCurve, S>(
        bases: &[G],
        scalars: &[S],
        start: usize,
        gpu_op: GPU_OP,
        ...
        let res = gpu::msm_gpu(bases, scalars, start, gpu_op, is_mont_form).unwrap();
