...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
    ...
    #[allow(clippy::type_complexity)]
    pub fn commit_plus_rand<'b>(
        saved_comm: &mut LabeledCommitment<Commitment<E>>,
        v_h_comm: &LabeledCommitment<Commitment<E>>,
        rand: &E::Fr,
        _terminator: &AtomicBool,
    ) -> Result<(), PCError> {
        let computed = v_h_comm.commitment.0.mul(*rand);
        let result = saved_comm.commitment.0.to_projective() + computed;

        saved_comm.commitment.0 = result.to_affine();
        Ok(())
    }
                    ...
                    .map(move |p| {
                        ...
                        match p {
                            ...
                            PolynomialWithBasis::Monomial { polynomial, degree_bound } => {
                                ...
                                // use `G` or use `SHIFTED_G` for commitment
                                let mut is_g = false;
