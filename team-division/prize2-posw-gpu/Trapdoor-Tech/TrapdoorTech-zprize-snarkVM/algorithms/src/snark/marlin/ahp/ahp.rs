...
use ec_gpu_common::{PolyKernel, GpuPolyContainer, PrimeField as GpuPrimeField};
...
impl<F: PrimeField, MM: MarlinMode> AHPForR1CS<F, MM> {
    ...
    /// Public input should be unformatted.
    #[allow(non_snake_case)]
    pub fn construct_linear_combinations_on_gpu<E: EvaluationsProvider<F>, Fg: GpuPrimeField>(
        kern: &PolyKernel<Fg>,
        ...
        gpu_powers.setup_powers(&beta)?;

        let mut z_b_s_at_beta = Vec::new();

        // Lincheck sumcheck:
        let z_b_s = (0..state.batch_size)
            .map(|i| {
                let z_b_i = witness_label("z_b", i);
                let gpu_zb = gpu_container.find(kern, &z_b_i).unwrap();
                z_b_s_at_beta.push(gpu_zb.evaluate_at_naive(&mut gpu_powers, &mut gpu_results, &beta).unwrap());
                gpu_container.save(&z_b_i, gpu_zb).unwrap();
        ...
        let batch_z_b_at_beta: F =
            z_b_s_at_beta.iter().zip_eq(batch_combiners).map(|(z_b_at_beta, combiner)| *z_b_at_beta * combiner).sum();
        let gpu_g1 = gpu_container.find(kern, "g_1")?;
        let g_1_at_beta = gpu_g1.evaluate_at_naive(&mut gpu_powers, &mut gpu_results, &beta)?;
        gpu_container.save("g_1", gpu_g1)?;
        ...
        gpu_powers.setup_powers(&gamma)?;
        let gpu_ga = gpu_container.find(kern, "g_a")?;
        let g_a_at_gamma = gpu_ga.evaluate_at_naive(&mut gpu_powers, &mut gpu_results, &gamma)?;
        gpu_container.save("g_a", gpu_ga)?;
        ...
        let gpu_gb = gpu_container.find(kern, "g_b")?;
        let g_b_at_gamma = gpu_gb.evaluate_at_naive(&mut gpu_powers, &mut gpu_results, &gamma)?;
        gpu_container.save("g_b", gpu_gb)?;
        ...
        let gpu_gc = gpu_container.find(kern, "g_c")?;
        let g_c_at_gamma = gpu_gc.evaluate_at_naive(&mut gpu_powers, &mut gpu_results, &gamma)?;
        gpu_container.save("g_c", gpu_gc)?;
