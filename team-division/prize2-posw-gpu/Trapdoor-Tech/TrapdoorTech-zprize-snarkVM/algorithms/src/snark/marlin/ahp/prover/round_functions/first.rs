...
impl<F: PrimeField, MM: MarlinMode> AHPForR1CS<F, MM> {
    ...
    pub fn prover_first_round<'a, R: RngCore>(
        ...
        let batches = job_pool
            ...
            .map(|(w, z_a, z_b)| {
                ...
                let (_, z_a_poly, z_a) = z_a.z_m().unwrap();
                let (_, z_b_poly, z_b) = z_b.z_m().unwrap();
    ...
    pub(crate) fn calculate_mask_poly<R: RngCore>(
    ...
    fn calculate_z_m<'a>(
        ...
        PoolResult::MatrixPoly(poly, poly_for_opening, poly_for_committing)
...
pub enum PoolResult<'a, F: PrimeField> {
    ...
    MatrixPoly(DensePolynomial<F>, LabeledPolynomial<F>, LabeledPolynomialWithBasis<'a, F>),
    // MatrixPoly(LabeledPolynomial<F>, LabeledPolynomialWithBasis<'a, F>),
}

impl<'a, F: PrimeField> PoolResult<'a, F> {
    pub(crate) fn witness(self) -> Option<LabeledPolynomial<F>> {
        match self {
    ...
    pub(crate) fn z_m(self) -> Option<(DensePolynomial<F>, LabeledPolynomial<F>, LabeledPolynomialWithBasis<'a, F>)> {
        match self {
            Self::MatrixPoly(p0, p1, p2) => Some((p0, p1, p2)),
            _ => None,
        }
    }

    // pub(crate) fn z_m(self) -> Option<(LabeledPolynomial<F>, LabeledPolynomialWithBasis<'a, F>)> {
    //     match self {
