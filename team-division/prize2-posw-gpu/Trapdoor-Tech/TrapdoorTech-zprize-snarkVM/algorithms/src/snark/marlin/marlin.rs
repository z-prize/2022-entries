...
use crate::{
    ...
    polycommit::sonic_pc::{
        Commitment, Evaluations, LabeledCommitment, LabeledPolynomialWithBasis, PolynomialWithBasis, Randomness,
        SonicKZG10,
    },
    snark::marlin::{
...
use crate::crypto_hash::sha256d_to_u64;
    ...
    pub static ref GPU_KERN_IDX: usize = {
        std::env::var("GPU_IDX")
            .and_then(|v| match v.parse() {
                Ok(val) => Ok(val),
                Err(_) => {
                    eprintln!("Invalid env GPU_IDX! Defaulting to 0...");
                    Ok(0)
                }
            })
            .unwrap_or(0)
    ...
    Input: ToConstraintField<E::Fr> + ?Sized,
{
    ...
    // gpu version of `prove_batch`
    #[allow(clippy::only_used_in_recursion)]
    fn prove_batch_with_terminator_on_gpu<C: ConstraintSynthesizer<E::Fr>, R: Rng + CryptoRng>(
        circuit_proving_key: &CircuitProvingKey<E, MM>,
        circuits: &[C],
        terminator: &AtomicBool,
        zk_rng: &mut R,
        ...
        block_height: u32,
        ctr: Option<&AtomicUsize>,
        tmp_proof: &mut Option<Self::Proof>,
    ) -> Result<Self::Proof, SNARKError> {
        let poly_kern = GPU_POLY_KERNEL.borrow_mut();
        let mut poly_container = GPU_POLY_CONTAINER.borrow_mut();

        let mut initialized = GPU_INITIALIZED.borrow_mut();

        if !*initialized {
            AHPForR1CS::<E::Fr, MM>::prepare_domain(&poly_kern, &mut poly_container, 1 << 15).unwrap();
            AHPForR1CS::<E::Fr, MM>::prepare_domain(&poly_kern, &mut poly_container, 1 << 16).unwrap();
            AHPForR1CS::<E::Fr, MM>::prepare_domain(&poly_kern, &mut poly_container, 1 << 17).unwrap();

            AHPForR1CS::<E::Fr, MM>::preload_arith_matrix(
                &poly_kern,
                &mut poly_container,
                &circuit_proving_key.circuit.a_arith,
                "a",
            )
            .unwrap();
            ...
            AHPForR1CS::<E::Fr, MM>::preload_arith_matrix(
                &poly_kern,
                &mut poly_container,
                &circuit_proving_key.circuit.b_arith,
                "b",
            )
            .unwrap();
            ...
            AHPForR1CS::<E::Fr, MM>::preload_arith_matrix(
                &poly_kern,
                &mut poly_container,
                &circuit_proving_key.circuit.c_arith,
                "c",
            )
            .unwrap();
        ...
        let saved_commit_g_a =
            AHPForR1CS::<E::Fr, MM>::commit_lagrange_tail::<GpuFr, E>(&poly_kern, &mut poly_container, 37908, 1 << 16)
                .unwrap();
        let saved_commit_g_b =
            AHPForR1CS::<E::Fr, MM>::commit_lagrange_tail::<GpuFr, E>(&poly_kern, &mut poly_container, 38031, 1 << 16)
                .unwrap();
        let saved_commit_g_c =
            AHPForR1CS::<E::Fr, MM>::commit_lagrange_tail::<GpuFr, E>(&poly_kern, &mut poly_container, 48434, 1 << 16)
                .unwrap();
        ...
        let prover_state_origin = AHPForR1CS::<_, MM>::init_prover(&circuit_proving_key.circuit, circuits)?;

        let public_input = prover_state_origin.public_inputs();
        let padded_public_input = prover_state_origin.padded_public_inputs();
        assert_eq!(prover_state_origin.batch_size, batch_size);

        let sponge_origin = Self::init_sponge(
            batch_size,
            &circuit_proving_key.circuit_verifying_key.circuit_commitments,
            &padded_public_input,
        );

        let mut saved_first_commitments = Vec::<LabeledCommitment<Commitment<E>>>::new();
        let mut saved_first_commitment_randomnesses = Vec::<Randomness<E>>::new();
        let mut saved_rand = E::Fr::zero();

        let mut first_time = true;

        let domain_h = prover_state_origin.constraint_domain;
        let v_h = domain_h.vanishing_polynomial();
        let poly_terms = vec![(E::Fr::one(), PolynomialWithBasis::new_sparse_monomial_basis(v_h, None))];
        let v_h = LabeledPolynomialWithBasis::new_linear_combination("v_h".to_string(), poly_terms, None);
        let v_h_vec = vec![v_h; 1];
        let v_h_comm = SonicKZG10::<E, FS>::commit(&circuit_proving_key.committer_key, v_h_vec, None)?.0[0].clone();

        let mut first_round_oracles_backup = Vec::new();
        let mut proof_final;

        loop {
            let prover_time = start_timer!(|| "Marlin::Prover");

            let mut sponge = sponge_origin.clone();
            let mut prover_state = prover_state_origin.clone();

            // --------------------------------------------------------------------
            // First round
            Self::terminate(terminator)?;

            let (mut first_round_oracles, my_first_round_oracles) =
                AHPForR1CS::<_, MM>::prover_first_round_on_gpu_saved(
                    &poly_kern,
                    &mut poly_container,
                    &mut prover_state,
                    zk_rng,
                    first_time,
                    &mut saved_rand,
                    &mut first_round_oracles_backup,
                )
                .unwrap();

            if first_time {
                first_round_oracles_backup.append(&mut my_first_round_oracles.clone());
            }

            Self::terminate(terminator)?;

            let first_round_comm_time = start_timer!(|| "Committing to first round polys");
            let (first_commitments, first_commitment_randomnesses) = if first_time {
                let (first_commitments, first_commitment_randomnesses) = {
                    SonicKZG10::<E, FS>::commit_on_gpu(
                        &poly_kern,
                        &mut poly_container,
                        &circuit_proving_key.committer_key,
                        first_round_oracles.iter_for_commit(),
                ...
                saved_first_commitments = first_commitments.clone();
                saved_first_commitment_randomnesses = first_commitment_randomnesses.clone();

                let index = saved_first_commitments
                    .iter()
                    .position(|poly| *poly.label() == "z_a_00000000".to_string())
                    .unwrap();
                let rand_computed = v_h_comm.commitment.0.mul(saved_rand);
                let result = saved_first_commitments[index].commitment.0.to_projective() - rand_computed;
                saved_first_commitments[index].commitment.0 = result.to_affine();

                (first_commitments, first_commitment_randomnesses)
            } else {
                let mut commitments = saved_first_commitments.clone();
                let index = commitments.iter().position(|poly| *poly.label() == "z_a_00000000".to_string()).unwrap();

                let mut poly_comm = commitments[index].clone();
                SonicKZG10::<E, FS>::commit_plus_rand(&mut poly_comm, &v_h_comm, &saved_rand, terminator)?;
                commitments[index] = poly_comm;

                (commitments, saved_first_commitment_randomnesses.clone())
            };
            end_timer!(first_round_comm_time);
            ...
            // Second round
            Self::terminate(terminator)?;
            let (second_oracles, prover_state) = AHPForR1CS::<_, MM>::prover_second_round_on_gpu(
                &poly_kern,
                &mut poly_container,
                &first_round_oracles,
            ...
            let (second_commitments, second_commitment_randomnesses) =
                SonicKZG10::<E, FS>::commit_with_terminator_on_gpu(
                    &poly_kern,
                    &mut poly_container,
                    &circuit_proving_key.committer_key,
                    second_oracles.iter().map(Into::into),
            ...
            let (prover_third_message, third_oracles, prover_state) = AHPForR1CS::<_, MM>::prover_third_round_on_gpu(
                &poly_kern,
                &mut poly_container,
                &verifier_second_msg,
                prover_state,
            ...
            let third_round_comm_time = start_timer!(|| "Committing to third round polys");
            let (mut third_commitments, third_commitment_randomnesses) =
                SonicKZG10::<E, FS>::commit_with_terminator_on_gpu(
                    &poly_kern,
                    &mut poly_container,
                    &circuit_proving_key.committer_key,
            ...
            let mut commit_g_a = saved_commit_g_a.clone();
            let mut commit_g_b = saved_commit_g_b.clone();
            let mut commit_g_c = saved_commit_g_c.clone();

            commit_g_a.mul_assign(prover_third_message.sum_a);
            commit_g_b.mul_assign(prover_third_message.sum_b);
            commit_g_c.mul_assign(prover_third_message.sum_c);

            commit_g_a.add_assign_mixed(&third_commitments[0].commitment.0);
            commit_g_b.add_assign_mixed(&third_commitments[1].commitment.0);
            commit_g_c.add_assign_mixed(&third_commitments[2].commitment.0);

            third_commitments[0].commitment.0 = commit_g_a.to_affine();
            third_commitments[1].commitment.0 = commit_g_b.to_affine();
            third_commitments[2].commitment.0 = commit_g_c.to_affine();
            ...
            let fourth_oracles = AHPForR1CS::<_, MM>::prover_fourth_round_on_gpu(
                &poly_kern,
                &mut poly_container,
                &verifier_third_msg,
                prover_state,
        ...
        let commitments = proof::Commitments {
            ...
            let lc_s = AHPForR1CS::<_, MM>::construct_linear_combinations_on_gpu(
                &poly_kern,
                &mut poly_container,
                &public_input,
                &polynomials,
            ...
            use crate::polycommit::sonic_pc::LCTerm;
            let mut gpu_powers = poly_container.ask_for_powers(&poly_kern).unwrap();
            let mut gpu_results = poly_container.ask_for_results(&poly_kern).unwrap();

            let mut evaluations = std::collections::BTreeMap::new();
            for (label, (_, point)) in query_set.to_set() {
                if !AHPForR1CS::<E::Fr, MM>::LC_WITH_ZERO_EVAL.contains(&label.as_str()) {
                    let lc = lc_s.get(&label).ok_or_else(|| AHPError::MissingEval(label.to_string()))?;
                    let mut eval_result = E::Fr::zero();
                    gpu_powers.setup_powers(&point).unwrap();

                    for (coeff, term) in lc.iter() {
                        let value = if let LCTerm::PolyLabel(label) = term {
                            let gpu_poly = poly_container.find(&poly_kern, &label).unwrap();
                            let res = gpu_poly.evaluate_at_naive(&mut gpu_powers, &mut gpu_results, &point).unwrap();
                            poly_container.save(label, gpu_poly).unwrap();
                            res
                        } else {
                            assert!(term.is_one());
                            E::Fr::one()
                        };
                        eval_result += &(*coeff * value)
                    }
                    evaluations.insert(label, eval_result);
                }
            }
            poly_container.recycle(gpu_powers).unwrap();
            poly_container.recycle(gpu_results).unwrap();
            ...
            let pc_proof = SonicKZG10::<E, FS>::open_combinations_on_gpu(
                &poly_kern,
                &mut poly_container,
                &circuit_proving_key.committer_key,
                lc_s.values(),
            ...
            proof_final = proof.clone();
            *tmp_proof = Some(proof.clone());

            match proof_final.to_bytes_le() {
                Ok(proof_bytes) => {
                    let hash_difficulty = sha256d_to_u64(&proof_bytes);
                    if hash_difficulty > target_difficulty {
                        eprintln!(
                            "PoSW difficulty target is not met. Expected {}, found {}, at block_height {}",
                            target_difficulty, hash_difficulty, block_height
                        );
                    } else {
                        println!("hash_difficulty met, expected {}, found {}", target_difficulty, hash_difficulty);
                        break;
                    }
                }
                Err(error) => {
                    eprintln!("Failed to convert PoSW proof to bytes: {}", error);
                    return Err(SNARKError::Message(String::from("convert PoSW proof error")));
                }
            };
