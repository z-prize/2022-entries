...
use std::{borrow::Borrow, fmt::Debug, sync::atomic::{AtomicBool, AtomicUsize}};
...
pub trait SNARK {
    ...
    fn prove_batch_with_terminator_on_gpu<C: ConstraintSynthesizer<Self::ScalarField>, R: Rng + CryptoRng>(
        proving_key: &Self::ProvingKey,
        input_and_witness: &[C],
        terminator: &AtomicBool,
        rng: &mut R,
    ...
    fn prove_with_terminator_on_gpu<C: ConstraintSynthesizer<Self::ScalarField>, R: Rng + CryptoRng>(
        proving_key: &Self::ProvingKey,
        input_and_witness: &C,
        terminator: &AtomicBool,
        rng: &mut R,
        ...
        Self::prove_batch_with_terminator_on_gpu(proving_key, std::slice::from_ref(input_and_witness), terminator, rng, target_difficulty, block_height, ctr, tmp_proof)
    }
