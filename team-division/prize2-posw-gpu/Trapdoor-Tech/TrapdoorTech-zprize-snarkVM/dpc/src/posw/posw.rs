...
pub fn reload_all_bases<G>()
where
    G: snarkvm_curves::AffineCurve,
{
    use ec_gpu_common::GPU_OP;
    use snarkvm_algorithms::msm::VariableBase;

    let v: Vec<G::ScalarField> = Vec::new();

    let bases: Vec<G> = Vec::new();

    VariableBase::trigger_reload_base::<G>();
    VariableBase::msm_gpu_reuse_base(&bases, v.as_slice(), 0, GPU_OP::SETUP_G, false);
    VariableBase::msm_gpu_reuse_base(&bases, v.as_slice(), 0, GPU_OP::SETUP_SHIFTED_G, false);
    VariableBase::msm_gpu_reuse_base(&bases, v.as_slice(), 0, GPU_OP::SETUP_LAGRANGE_G, false);
    VariableBase::msm_gpu_reuse_base(&bases, v.as_slice(), 0, GPU_OP::SETUP_SHIFTED_LAGRANGE_G, false);
}
...
impl<N: Network> PoSW<N> {
    pub fn prove_once_unchecked_ctr<R: Rng + CryptoRng>(
        &self,
        ...
        block_height: u32,
        tmp_proof: &mut Option<<<N as Network>::PoSWSNARK as SNARK>::Proof>,
        ...
        // Construct a PoSW proof that meets target difficulty.
        Ok(PoSWProof::<N>::new(
            <<N as Network>::PoSWSNARK as SNARK>::prove_with_terminator_on_gpu(
                pk,
                circuit,
                terminator,
                rng,
...
impl<N: Network> PoSWScheme<N> for PoSW<N> {
    ...
    fn mine<R: Rng + CryptoRng>(
        ...
        loop {
            ...
            let proof = self.prove_once_unchecked_ctr(
                &mut circuit,
                terminator,
                &ctr,
                rng,
...
mod tests {
    ...
    use crate::{reload_all_bases, testnet2::Testnet2, BlockTemplate, Network, PoSWScheme};
    ...
    fn test_posw_marlin() {
        ...
        reload_all_bases::<<<Testnet2 as Network>::InnerCurve as PairingEngine>::G1Affine>();
