...
impl<E: PairingEngine, S: FiatShamirRng<E::Fr, E::Fq>> SonicKZG10<E, S> {
        ...
        Commitment<E>: 'a,
    {
        ...
        let mut lc_commitments_aff = Vec::new();
        ...
        for lc in linear_combinations {
            ...
            for (coeff, label) in lc.iter().filter(|(_, l)| !l.is_one()) {
                ...
                let &(cur_poly, _cur_rand, cur_comm) =
                    label_map.get(label as &str).ok_or(PCError::MissingPolynomial { label: label.to_string() })?;
                if num_polys == 1 {
                    poly = cur_poly.polynomial().as_dense().unwrap().clone();
                    lc_commitments_aff.push(cur_comm.commitment().clone());
        ...
        lc_commitments_aff.append(&mut comms.into_iter().map(|c| kzg10::Commitment(c)).collect::<Vec<_>>());
        let lc_commitments = lc_info
            .into_iter()
            .zip_eq(lc_commitments_aff.into_iter())
