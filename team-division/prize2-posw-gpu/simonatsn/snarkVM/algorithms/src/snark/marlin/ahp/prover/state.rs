...
pub struct State<'a, F: PrimeField, MM: MarlinMode> {
    ...
    pub padded_public_variables: Vec<Vec<F>>,
    ...
    /// The length of this list must be equal to the batch size.
    pub z_b: Option<Vec<Vec<F>>>,

    /// A list of polynomials corresponding to the interpolation of the public input.
    /// The length of this list must be equal to the batch size.
    pub x_poly: Vec<DensePolynomial<F>>,

    /// The first round oracles sent by the prover.
    /// The length of this list must be equal to the batch size.
    pub first_round_oracles: Option<Arc<super::FirstOracles<'a, F>>>,

    /// Randomizers for z_b.
    /// The length of this list must be equal to the batch size.
    pub mz_poly_randomizer: Option<Vec<F>>,

    /// The challenges sent by the verifier in the first round
    pub verifier_first_message: Option<verifier::FirstMessage<F>>,

    /// Polynomials involved in the holographic sumcheck.
    pub lhs_polynomials: Option<[DensePolynomial<F>; 3]>,
    /// Polynomials involved in the holographic sumcheck.
    pub sums: Option<[F; 3]>,
}
