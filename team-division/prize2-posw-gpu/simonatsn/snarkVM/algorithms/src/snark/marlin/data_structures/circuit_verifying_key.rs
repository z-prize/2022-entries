...
use anyhow::Result;
use core::{fmt, marker::PhantomData, str::FromStr, cell::RefCell};
...
impl<E: PairingEngine, MM: MarlinMode> Prepare for CircuitVerifyingKey<E, MM> {
    ...
    fn prepare(&self) -> Self::Prepared {
        let first_comm_x = self.circuit_commitments[0].0.to_x_coordinate();
        let hm  = generic_singleton::get_or_init!(||
            RefCell::new(HashMap::<E::Fq, Self::Prepared>::new())
        );
        if hm.borrow().contains_key(&first_comm_x) {
            return hm.borrow().get(&first_comm_x).unwrap().clone();
        }
        ...
        let pvk = PreparedCircuitVerifyingKey {
        ...
        hm.borrow_mut().insert(first_comm_x, pvk.clone());
