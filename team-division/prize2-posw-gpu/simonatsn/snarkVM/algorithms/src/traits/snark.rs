...
pub trait SNARK {
    ...
    type PartialProof: Clone + Send + Sync;
    ...
    fn prove_batch_with_terminator_test<C: ConstraintSynthesizer<Self::ScalarField>, R: Rng + CryptoRng>(
        _proving_key: &Self::ProvingKey,
        _input_and_witness: &[C],
    ...
    fn prove_with_terminator<C: ConstraintSynthesizer<Self::ScalarField>, R: Rng + CryptoRng>(
        ...
        Self::prove_batch_with_terminator_test(proving_key, std::slice::from_ref(input_and_witness), terminator, rng)
    }
    ...
    fn prove_second_half(
        _proving_key: &Self::ProvingKey,
        _terminator: &AtomicBool,
        _partial_proof: &Self::PartialProof,
        _buffer: &Arc<(Mutex<VecDeque<Result<Self::Proof, SNARKError>>>, Condvar)>,
    ) -> Result<Self::Proof, SNARKError> {
        unimplemented!()
    }
