...
use crate::traits::posw::PoSWScheme;
use rand::thread_rng;

use snarkvm_utilities::sync::{Arc, Mutex, Condvar};
use snarkvm_utilities::collections::VecDeque;
...
pub struct PoSWCircuit<N: Network> {
    ...
    pub partial_proof: Option<<<N as Network>::PoSWSNARK as SNARK>::PartialProof>,
    pub buffer: Option<Arc<(Mutex<VecDeque<Result<<<N as Network>::PoSWSNARK as SNARK>::Proof, SNARKError>>>, Condvar)>>,
}

impl<N: Network> Drop for PoSWCircuit<N> {
    fn drop(&mut self) {
        snarkvm_cuda::cleanup_gpu();
    }
}
...
impl<N: Network> PoSWCircuit<N> {
    ...
    pub fn new(block_template: &BlockTemplate<N>, nonce: N::PoSWNonce) -> Result<Self> {
        ...
        let mut circuit = Self { block_header_root: (*tree.root()).into(), nonce, hashed_leaves: tree.hashed_leaves().to_vec(), partial_proof: None , buffer: None };

        let terminator = AtomicBool::new(false);
        let mut rng = thread_rng();

        let proof = N::posw().setup_mine(&mut circuit, &terminator, &mut rng)?;
        circuit.partial_proof = Some(proof);

        circuit.buffer = Some(Arc::new((Mutex::new(VecDeque::new()), Condvar::new())));
