...
impl<N: Network> PoSWScheme<N> for PoSW<N> {
    ...
    fn setup_mine<R: Rng + CryptoRng>(
        &self,
        circuit: &mut PoSWCircuit<N>,
        terminator: &AtomicBool,
        rng: &mut R,
    ) -> Result<<<N as Network>::PoSWSNARK as SNARK>::PartialProof, PoSWError> {
        ...
        // Circuit dependent first portion of proof, becomes input to posw for this block
        let pp = <<N as Network>::PoSWSNARK as SNARK>::prove_first_half(pk, std::slice::from_ref(circuit), terminator, rng)?;

        <<N as Network>::PoSWSNARK as SNARK>::cache_committer_key(pk);
    ...
    fn prove_once_unchecked<R: Rng + CryptoRng>(
        ...
        let pk = self.proving_key.as_ref().expect("tried to mine without a PK set up");
        let b = Arc::clone(&circuit.buffer.as_ref().unwrap());
        let p = <<N as Network>::PoSWSNARK as SNARK>::prove_second_half(pk, terminator, &circuit.partial_proof.as_ref().unwrap(), &b)?;

        Ok(PoSWProof::<N>::new(p.into()))
    }
