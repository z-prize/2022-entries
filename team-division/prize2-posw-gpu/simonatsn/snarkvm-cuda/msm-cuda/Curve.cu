...
class G1Montgomery {
  ...
  __device__ __forceinline__ static void sqr(EvenOdd& evenOdd, Value& temp, bool& carry, const Value& a, const Value& n) {
    carry=mp_sqr_red_cl<G1Montgomery, limbs>(evenOdd, temp, a, n);
  }
...
class PointXYZZ {
  ...
  __device__ __forceinline__ void assign(const FieldValue& xValue, const FieldValue& yValue, const FieldValue& zzValue, const FieldValue& zzzValue) {
    Field::set(x, xValue);
    Field::set(y, yValue);
    Field::set(zz, zzValue);
    Field::set(zzz, zzzValue);
  ...
  __device__ __forceinline__ void load(void* ptr) {
    Field::load(x, byteOffset(ptr, Field::bytes*0));
    Field::load(y, byteOffset(ptr, Field::bytes*1));
    Field::load(zzz, byteOffset(ptr, Field::bytes*2));
    Field::load(zz, byteOffset(ptr, Field::bytes*3));
  }

  __device__ __forceinline__ void loadUnaligned(void* ptr) {
    Field::loadUnaligned(x, byteOffset(ptr, Field::bytes*0));
    Field::loadUnaligned(y, byteOffset(ptr, Field::bytes*1));
    Field::loadUnaligned(zzz, byteOffset(ptr, Field::bytes*2));
    Field::loadUnaligned(zz, byteOffset(ptr, Field::bytes*3));
  }
  
  __device__ __forceinline__ void store(void* ptr) {
    Field::store(byteOffset(ptr, Field::bytes*0), x);
    Field::store(byteOffset(ptr, Field::bytes*1), y);
    Field::store(byteOffset(ptr, Field::bytes*2), zzz);
    Field::store(byteOffset(ptr, Field::bytes*3), zz);
  }
...
class HighThroughput {
  ...
  __device__ __forceinline__ void add(const PointXY<Field> point, bool valid=true) {
    ...
    uint32_t     state;
    bool         carry, done, dbl, uniformAffine, square=false;
