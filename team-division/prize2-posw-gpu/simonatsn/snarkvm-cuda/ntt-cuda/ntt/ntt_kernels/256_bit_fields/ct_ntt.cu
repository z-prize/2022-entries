             ...
             const bool is_intt, const fr_t d_domain_size_inverse)
{
    ...
    for (int s = 6; s < iterations; s++) {
        ...
        unsigned int thrdMask = (1 << s) - 1;
        unsigned int rank = threadIdx.x & thrdMask;
        unsigned int block = threadIdx.x & ~thrdMask;
        bool pos = rank < laneMask;
        ...
        __syncthreads();
        shared_exchange[block + rank] = r0;
        __syncthreads();
        r0 = shared_exchange[block + (rank ^ laneMask)];
        __syncthreads();
        shared_exchange_idx[block + rank] = idx0;
        __syncthreads();
        idx0 = shared_exchange_idx[block + (rank ^ laneMask)];
...
#define NTT_ARGUMENTS \
        ...
        const fr_t*, const fr_t*, const fr_t*, const fr_t*, unsigned int, \
        bool, fr_t
