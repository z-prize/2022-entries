...
void bit_rev_permutation_aux(fr_t* out, const fr_t* in, uint32_t lg_domain_size)
{
    ...
    fr_t (*xchg)[4][4] = reinterpret_cast<decltype(xchg)>(exchange);

    index_t step = (index_t)1 << (lg_domain_size - 2);
    index_t group_idx = (threadIdx.x + blockDim.x * (index_t)blockIdx.x) >> 2;
    uint32_t brev_limit = lg_domain_size - 4;
    index_t brev_mask = ((index_t)1 << brev_limit) - 1;
    index_t group_idx_brev =
        (group_idx & ~brev_mask) | bit_rev(group_idx & brev_mask, brev_limit);
    uint32_t group_thread = threadIdx.x & 0x3;
    uint32_t group_thread_rev = bit_rev(group_thread, 2);
    uint32_t group_in_block_idx = threadIdx.x >> 2;

    #pragma unroll
    for (uint32_t i = 0; i < 4; i++) {
        xchg[group_in_block_idx][i][group_thread_rev] =
            in[group_idx * 4 + i * step + group_thread];
    }
    ...
    #pragma unroll
    for (uint32_t i = 0; i < 4; i++) {
        out[group_idx_brev * 4 + i * step + group_thread] =
            xchg[group_in_block_idx][group_thread_rev][i];
    }
}

__device__ __forceinline__
fr_t get_intermediate_root(index_t pow, const fr_t* rp,
                           unsigned int nbits = MAX_LG_DOMAIN_SIZE)
{
#if (__CUDACC_VER_MAJOR__-0) >= 11
    __builtin_assume(nbits <= MAX_LG_DOMAIN_SIZE);
#endif

    const fr_t (*roots)[WINDOW_SIZE] = reinterpret_cast<decltype(roots)>(rp);
    unsigned int off = (nbits - 1) / LG_WINDOW_SIZE;
    unsigned int win = off * LG_WINDOW_SIZE;

    fr_t root = roots[off][(pow >> win)];
    #pragma unroll
    while (off--) {
        win -= LG_WINDOW_SIZE;
        root *= roots[off][(pow >> win) % WINDOW_SIZE];
    }
...
__global__
void LDE_distribute_powers(fr_t* d_inout, uint32_t lg_blowup,
                           const fr_t* gen_powers)
{
    index_t idx = threadIdx.x + blockDim.x * (index_t)blockIdx.x;
    fr_t r = d_inout[idx];

    r *= get_intermediate_root(idx << lg_blowup, gen_powers);

    d_inout[idx] = r;
}

__global__
void LDE_spread_distribute_powers(fr_t* out, fr_t* in, const fr_t* gen_powers,
...
# include "256_bit_fields/gs_ntt.cu"
# include "256_bit_fields/ct_ntt.cu"
