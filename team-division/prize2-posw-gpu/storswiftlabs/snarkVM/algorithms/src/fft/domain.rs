...
impl<F: FftField> EvaluationDomain<F> {
    ...
    fn fft_helper_in_place_with_pc<T: DomainCoeff<F>>(
        ...
        if ord == OI {
            if crate::msm::VariableBase::working_gpu(x_s.len()) {
                crate::msm::VariableBase::msm4_oi_helper(x_s, self.group_gen);
            } else {
            ...
            if crate::msm::VariableBase::working_gpu(x_s.len()) {
                crate::msm::VariableBase::msm4_io_helper(x_s, self.group_gen);
            } else {
                let pc = pre_comp.precomputation_for_subdomain(self).unwrap();
        ...
        if ord == II {
            ...
            derange_helper2(x_s, log_len);
        }

        if std::env::var("ENABLE_FFT_LOG").is_ok() {
            eprintln!(
                "{}  fft {}, use {}",
                Local::now().to_rfc3339(), x_s.len(), Local::now().timestamp_millis() - a0);
        }
    }
    ...
    fn ifft_helper_in_place_with_pc<T: DomainCoeff<F>>(
        ...
        if ord == IO {
            if crate::msm::VariableBase::working_gpu(x_s.len()) {
                crate::msm::VariableBase::msm4_io_helper(x_s, self.group_gen_inv);
            } else {
                let pc = pre_comp.precomputation_for_subdomain(self).unwrap();
            ...
            if crate::msm::VariableBase::working_gpu(x_s.len()) {
                crate::msm::VariableBase::msm4_oi_helper(x_s, self.group_gen_inv);
            } else {
                let pc = pre_comp.precomputation_for_subdomain(self).unwrap();
    ...
    #[allow(unused)]
    #[inline(always)]
    pub fn butterfly_fn_io<T: DomainCoeff<F>>(((lo, hi), root): ((&mut T, &mut T), &F)) {
        let neg = *lo - *hi;
        *lo += *hi;
        *hi = neg;
        *hi *= *root;
    ...
    #[allow(unused)]
    #[inline(always)]
    pub fn butterfly_fn_oi<T: DomainCoeff<F>>(((lo, hi), root): ((&mut T, &mut T), &F)) {
        *hi *= *root;
        let neg = *lo - *hi;
        *lo += *hi;
        *hi = neg;
    ...
    pub fn apply_butterfly<T: DomainCoeff<F>, G: Fn(((&mut T, &mut T), &F)) + Copy + Sync + Send>(
        g: G,
        xi: &mut [T],
        roots: &[F],
        step: usize,
    ...
    fn oi_helper_with_roots<T: DomainCoeff<F>>(&self, xi: &mut [T], roots_cache: &[F]) {
        ...
        while gap < xi.len() {
            ...
            // the roots lookup is done a significant amount of times
            // Which also implies a large lookup stride.
            let (roots, step) = if num_chunks >= MIN_NUM_CHUNKS_FOR_COMPACTION && chunk_size < len {
...
fn derange_helper2<T>(xi: &mut [T], log_len: u32) {
    let log_len = 64 - log_len;
    for idx in 1..(xi.len() as u64 - 1) {
        let ridx = idx.swap_bits() >> log_len;
        if idx < ridx {
            xi.swap(idx as usize, ridx as usize);
        }
    }
