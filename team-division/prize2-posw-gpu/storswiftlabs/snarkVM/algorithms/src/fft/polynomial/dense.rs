...
use crate::fft::domain::{log2, compute_powers_serial};
...
*/
        let powers_of_point = self.roots_of_unity2(point);
        let zero = F::zero();
    ...
    /// Evaluates `self` at the given `point` in the field.
    pub fn evaluate2(&self, powers_of_point: &Vec<F>) -> F {
        if self.is_zero() {
            return F::zero();
        } else if powers_of_point.is_empty() {
            return self.coeffs[0];
        }
    ...
    #[cfg(feature = "parallel")]
    pub fn roots_of_unity2(&self, root: F) -> Vec<F> {
        ...
        if log_size <= LOG_ROOTS_OF_UNITY_PARALLEL_SIZE {
            compute_powers_serial(size, root)
        } else {
            let mut temp = root;
    ...
    #[cfg(feature = "parallel")]
    fn roots_of_unity_recursive2(out: &mut [F], log_powers: &[F]) {
