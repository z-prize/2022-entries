...
impl<'a, F: Field> Polynomial<'a, F> {
    ...
    #[inline]
    pub fn evaluate2(&self, point: &Vec<F>) -> F {
        match self {
            Sparse(p) => p.evaluate2(point),
            Dense(p) => p.evaluate2(point),
        }
    }
    ...
    pub fn divide_with_q_and_r(&self, divisor: &Self) -> Option<(DensePolynomial<F>, DensePolynomial<F>)> {
        if self.is_zero() {
            ...
            if let Sparse(p) = divisor {
                ...
                let divsize = p.degree();
                coeffs.remove(&divsize);

                lastc = remainder.degree() + 1;
                if divisor_leading_inv == F::one() {
                    while lastc > divsize {
                        lastc -= 1;
                        let cur_q_coeff = remainder.coeffs[lastc];
                        let cur_q_degree = lastc - divsize;
                        for (i, div_coeff) in coeffs.iter() {
                    ...
                    while lastc > divsize {
                        lastc -= 1;
                        let cur_q_coeff = remainder.coeffs[lastc] * divisor_leading_inv;
                        remainder.coeffs[lastc] = cur_q_coeff;
                        let cur_q_degree = lastc - divsize;
                        for (i, div_coeff) in coeffs.iter() {
                            remainder[cur_q_degree + i] -= &(cur_q_coeff * div_coeff);
                        }
                ...
                if divisor_leading_inv == F::one() {
                    while lastc > divsize {
                        ...
                        let cur_q_coeff = remainder.coeffs[lastc];
                        let cur_q_degree = lastc - divsize;
                        for (i, div_coeff) in coeffs.iter().enumerate() {
                            remainder[cur_q_degree + i] -= &(cur_q_coeff * div_coeff);
                        }
                    }
                } else {
            ...
            let ret = remainder.coeffs.split_at(lastc);
            Some((
                DensePolynomial::from_coefficients_slice(ret.1),
                DensePolynomial::from_coefficients_slice(ret.0)))
        }
    }
}
