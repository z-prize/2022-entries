...
#[inline]
fn batched_window<G: AffineCurve, F: BigInteger>(
    bases: &[G],
    scalars: &[F],
    w_start: usize,
    c: usize,
...
pub fn msm<G: AffineCurve, F: BigInteger>(bases: &[G], scalars: &[F]) -> G::Projective {
    ...
    let c = match scalars.len() < 32 {
        true => 1,
        false => { if scalars.len() > 32768 { 12 } else { 10 } },
    ...
    let result  = window_sums.iter().rev().fold(G::Projective::zero(), |mut total, (sum_i, window_size)| {
        total += sum_i;
