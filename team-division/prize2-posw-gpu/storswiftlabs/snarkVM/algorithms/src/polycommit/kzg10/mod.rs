...
impl<E: PairingEngine> KZG10<E> {
    ...
    pub fn commit(
        ...
        let mut commitment = match polynomial {
            Polynomial::Dense(polynomial) => {
                let (num_leading_zeros, plain_coeffs) = skip_leading_zeros_and_convert_to_bigints2(polynomial);

                let msm_time = start_timer!(|| "MSM to compute commitment to plaintext poly");
                let commitment = VariableBase::msm2(&powers.powers_of_beta_g[num_leading_zeros..], plain_coeffs);
                end_timer!(msm_time);
        ...
        if randomness.blinding_polynomial.coeffs.len() > 0 {
            let msm_time = start_timer!(|| format!("MSM to compute commitment to random poly {}", randomness.blinding_polynomial.coeffs.len(),));
            let random_commitment =
                VariableBase::msm2(&powers.powers_of_beta_times_gamma_g, &randomness.blinding_polynomial.coeffs).to_affine();
            end_timer!(msm_time);
    ...
    pub fn commit_lagrange(
        ...
        let msm_time = start_timer!(|| format!("MSM lagrange {}", evaluations.len(),));
        let mut commitment = VariableBase::msm2(&lagrange_basis.lagrange_basis_at_beta_g, &evaluations);
        end_timer!(msm_time);
        ...
        if randomness.blinding_polynomial.coeffs.len() > 0 {
            // let random_ints = convert_to_bigints(&randomness.blinding_polynomial.coeffs);
            let msm_time = start_timer!(|| format!("MSM to compute lagrange commitment to random poly {}", randomness.blinding_polynomial.coeffs.len(),));
            let random_commitment =
                VariableBase::msm2(&lagrange_basis.powers_of_beta_times_gamma_g, &randomness.blinding_polynomial.coeffs).to_affine();
            end_timer!(msm_time);
    ...
    pub(crate) fn open_with_witness_polynomial(
        ...
        let (num_leading_zeros, witness_coeffs) = skip_leading_zeros_and_convert_to_bigints2(witness_polynomial);

        let witness_comm_time = start_timer!(|| "Computing commitment to witness polynomial");
        let mut w = VariableBase::msm2(&powers.powers_of_beta_g[num_leading_zeros..], witness_coeffs);
        ...
        let random_v = if let Some(hiding_witness_polynomial) = hiding_witness_polynomial {
            ...
            let witness_comm_time = start_timer!(|| format!("MSM to compute commitment to random witness polynomial {}", hiding_witness_polynomial.coeffs.len(),));
            w += &VariableBase::msm2(&powers.powers_of_beta_times_gamma_g, &hiding_witness_polynomial.coeffs);
            end_timer!(witness_comm_time);
...
fn skip_leading_zeros_and_convert_to_bigints2<F: PrimeField>(p: &DensePolynomial<F>) -> (usize, &[F]) {
    if p.coeffs.is_empty() {
        (0, &[])
    } else {
        let mut num_leading_zeros = 0;
