...
impl<F: PrimeField, MM: MarlinMode> AHPForR1CS<F, MM> {
    ...
    /// Public input should be unformatted.
    #[allow(non_snake_case)]
    pub fn construct_linear_combinations2(
        public_inputs: &[Vec<F>],
        evals: &[F],
        prover_third_message: &prover::ThirdMessage<F>,
        state: &verifier::State<F, MM>,
        ...
        //let z_b_s_at_beta = z_b_s.iter().map(|_| evals[0]).collect::<Result<Vec<_>, _>>()?;
        let z_b_s_at_beta = vec![evals[0]; state.batch_size];
        let batch_z_b_at_beta: F =
            z_b_s_at_beta.iter().zip_eq(batch_combiners).map(|(z_b_at_beta, combiner)| *z_b_at_beta * combiner).sum();
        let g_1_at_beta =  evals[1];//evals.get_lc_eval(&g_1, beta)?;
       ...
       // debug_assert!(evals.get_lc_eval(&lincheck_sumcheck, beta)?.is_zero());
        ...
        let g_a = LinearCombination::new("g_a", [(F::one(), "g_a")]);
        let g_a_at_gamma = evals[2];//evals.get_lc_eval(&g_a, gamma)?;
        let selector_a = largest_non_zero_domain.evaluate_selector_polynomial(non_zero_a_domain, gamma);
        let lhs_a =
            Self::construct_lhs("a", alpha, beta, gamma, v_H_at_alpha * v_H_at_beta, g_a_at_gamma, *sum_a, selector_a);
        matrix_sumcheck += &lhs_a;
        ...
        let g_b = LinearCombination::new("g_b", [(F::one(), "g_b")]);
        let g_b_at_gamma =  evals[3];//evals.get_lc_eval(&g_b, gamma)?;
        let selector_b = largest_non_zero_domain.evaluate_selector_polynomial(non_zero_b_domain, gamma);
        let lhs_b =
            Self::construct_lhs("b", alpha, beta, gamma, v_H_at_alpha * v_H_at_beta, g_b_at_gamma, *sum_b, selector_b);
        matrix_sumcheck += (r_b, &lhs_b);
        ...
        let g_c = LinearCombination::new("g_c", [(F::one(), "g_c")]);
        let g_c_at_gamma =  evals[4];//evals.get_lc_eval(&g_c, gamma)?;
        let selector_c = largest_non_zero_domain.evaluate_selector_polynomial(non_zero_c_domain, gamma);
        let lhs_c =
            Self::construct_lhs("c", alpha, beta, gamma, v_H_at_alpha * v_H_at_beta, g_c_at_gamma, *sum_c, selector_c);
        matrix_sumcheck += (r_c, &lhs_c);
    ...
    /// Public input should be unformatted.
    #[allow(non_snake_case)]
    pub fn msm4_evaluate_cpu<E: EvaluationsProvider<F>>(
        evals: &E,
        beta2: &Vec<F>,
        gamma2: &Vec<F>,
    ) -> Result<Vec<F>, AHPError> {

        let mut lc_eval = Vec::with_capacity(5);

        // Outer sumcheck:
        //let z_b_i = witness_label("z_b", 0);
        let z_b = LinearCombination::new("z_b_00000000", vec![(F::one(),"z_b_00000000")]);
        let g_1 = LinearCombination::new("g_1", vec![(F::one(), "g_1")]);

        let z_b_at_beta = evals.get_lc_eval2(&z_b, beta2)?;
        let g_1_at_beta = evals.get_lc_eval2(&g_1, beta2)?;

        lc_eval.push(z_b_at_beta);
        lc_eval.push(g_1_at_beta);

        let g_a = LinearCombination::new("g_a", vec![(F::one(), "g_a")]);
        let g_a_at_gamma = evals.get_lc_eval2(&g_a, gamma2)?;

        let g_b = LinearCombination::new("g_b", vec![(F::one(), "g_b")]);
        let g_b_at_gamma = evals.get_lc_eval2(&g_b, gamma2)?;

        let g_c = LinearCombination::new("g_c", vec![(F::one(), "g_c")]);
        let g_c_at_gamma = evals.get_lc_eval2(&g_c, gamma2)?;

        lc_eval.push(g_a_at_gamma);
        lc_eval.push(g_b_at_gamma);
        lc_eval.push(g_c_at_gamma);
...
pub trait EvaluationsProvider<F: PrimeField>: core::fmt::Debug {
    ...
    fn get_lc_eval2(&self, lc: &LinearCombination<F>, point: &Vec<F>) -> Result<F, AHPError>;
}
...
impl<'a, F: PrimeField> EvaluationsProvider<F> for crate::polycommit::sonic_pc::Evaluations<'a, F> {
    ...
    fn get_lc_eval2(&self, lc: &LinearCombination<F>, point: &Vec<F>) -> Result<F, AHPError> {
        let key = (lc.label.clone(), point[0]);
        self.get(&key).copied().ok_or_else(|| AHPError::MissingEval(lc.label.clone()))
    }
}
