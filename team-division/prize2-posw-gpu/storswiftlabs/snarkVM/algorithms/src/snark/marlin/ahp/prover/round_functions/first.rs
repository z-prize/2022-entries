...
impl<F: PrimeField, MM: MarlinMode> AHPForR1CS<F, MM> {
    ...
    fn calculate_w<'a>(
        ...
        let w_poly_time1 = start_timer!(|| "Computing w-1 polynomial");
        let constraint_domain = state.constraint_domain;
        ...
        end_timer!(w_poly_time1);

        let w_poly_time2 = start_timer!(|| "Computing w-2 polynomial");
        let w_poly_time2_1 = start_timer!(|| "Computing w-2-1 polynomial");
        let w_poly_evals = cfg_into_iter!(0..constraint_domain.size())
            ...
            .collect();
        end_timer!(w_poly_time2_1);
        let w_poly_time2_2 = start_timer!(|| "Computing w-2-2 polynomial");
    ...
    fn calculate_z_m<'a>(
        ...
        let poly_time_1 = start_timer!(|| format!("Computing {label} 1"));
        let evals = EvaluationsOnDomain::from_vec_and_domain(evaluations, constraint_domain);
        end_timer!(poly_time_1);
        let poly_time_2 = start_timer!(|| format!("Computing {label} 2"));
        let mut poly = evals.interpolate_with_pc_by_ref(state.ifft_precomputation());
        if should_randomize {
            poly += &(&v_H * r.unwrap());
        }
        end_timer!(poly_time_2);
        let poly_time_3 = start_timer!(|| format!("Computing {label} 3"));
        debug_assert!(
        ...
        end_timer!(poly_time_3);
        let poly_time_4 = start_timer!(|| format!("Computing {label} 4"));
        ...
        end_timer!(poly_time_4);
        let poly_time_5 = start_timer!(|| format!("Computing {label} 5"));
